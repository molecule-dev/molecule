...
describe('@molecule/api-ci-github-actions', () => {
  describe('toYAML', () => {
    ...
    describe('objects', () => {
      it('should stringify empty objects as {}', () => {
        ...
        const yaml = toYAML(config)
        // Empty object rendered inline (own-line `{}` is invalid YAML)
  ...
  describe('generateWorkflow', () => {
    it('should add header comment to generated workflow', () => {
      ...
      // The header must reference a regeneration path that actually exists —
      // it used to teach `npx molecule ci generate`, a command no CLI provides.
      expect(yaml).toContain('generateWorkflow()')
      expect(yaml).not.toContain('npx molecule ci generate')
    })
