/**
 * Type definitions for the drug-database core interface.
 *
 * Defines the canonical, provider-agnostic shapes used by every
 * drug-database provider (RxNorm, openFDA, etc.). Provider responses are
 * normalized so application code can swap providers with no behavioural
 * change.
 *
 * @module
 */
 ...
 */
export type DrugInteractionSeverity = 'unknown' | 'low' | 'moderate' | 'high'
 ...
 */
export interface DrugDatabaseProvider {
  /**
