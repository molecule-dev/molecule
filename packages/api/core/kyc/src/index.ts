 ...
 * @example
 * ```typescript
 * import type { KycWebhookHeaders } from '@molecule/api-kyc'
 * import { createVerificationSession, processWebhook, setProvider } from '@molecule/api-kyc'
 * import { createProvider } from '@molecule/api-kyc-stripe-identity'
 *
 * // Startup (server only): bond one provider. Secrets come from the server env.
 * setProvider(
 *   createProvider({
 ...
 * })
 * const redirectTo = session.url // persist session.sessionId against the user too
 *
 ...
 * async function handleKycWebhook(headers: KycWebhookHeaders, rawBody: Buffer) {
