...
const stub = (name = 'mock'): AIAgentsProvider => ({ name }) as AIAgentsProvider

describe('ai-agents provider singleton', () => {
  beforeEach(async () => {
    vi.resetModules()
  ...
  it('requireProvider throws when nothing is bonded', () => {
    expect(() => requireProvider()).toThrow(/AIAgents provider not configured/)
  })
