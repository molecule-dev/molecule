...
const stub = (name = 'mock'): AITranslationProvider => ({ name }) as AITranslationProvider

describe('ai-translation provider singleton', () => {
  beforeEach(async () => {
    vi.resetModules()
  ...
  it('requireProvider throws when nothing is bonded', () => {
    expect(() => requireProvider()).toThrow(/AITranslation provider not configured/)
  })
