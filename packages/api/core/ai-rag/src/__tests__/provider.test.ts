...
const stub = (name = 'mock'): AIRagProvider => ({ name }) as AIRagProvider

describe('ai-rag provider singleton', () => {
  beforeEach(async () => {
    vi.resetModules()
  ...
  it('requireProvider throws when nothing is bonded', () => {
    expect(() => requireProvider()).toThrow(/AIRag provider not configured/)
  })
