 ...
 * @example
 * ```typescript
 * import { createLabel, createShipment, setProvider, trackPackage } from '@molecule/api-shipping'
 * // Reads EASYPOST_API_KEY from the environment on every request (use a TEST key in dev).
 * import { provider } from '@molecule/api-shipping-easypost'
 *
 * // Startup: bond exactly one provider before any shipping call.
 * setProvider(provider)
 *
 * // 1. Create the shipment: quotes rates AND returns the shipmentId a purchase needs.
 ...
 * })
 *
 * // 2. Buy a label for a QUOTED rate — pass the exact ShippingRate object (it carries rateId).
 ...
 * console.log(status.status) // 'pre_transit' | 'in_transit' | 'delivered' | ...
 * ```
 *
 * @remarks
 * - **Bond first.** Every call throws "Shipping provider not configured" until
 *   `setProvider(provider)` runs. The provider's secret (e.g. `EASYPOST_API_KEY`,
 *   `SHIPPO_API_KEY`) comes from the environment, not from `setProvider`.
