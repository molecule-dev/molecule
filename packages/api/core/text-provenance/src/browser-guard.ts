/**
 * Browser guard — `@molecule/api-text-provenance` is SERVER-ONLY.
 *
...
if (g.window !== undefined && g.document !== undefined && !g.process?.versions?.node) {
  throw new Error(
    '@molecule/api-text-provenance is SERVER-ONLY: it was bundled into browser/client code. Import it only ' +
