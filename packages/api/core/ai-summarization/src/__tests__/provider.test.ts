...
const stub = (name = 'mock'): AISummarizationProvider => ({ name }) as AISummarizationProvider

describe('ai-summarization provider singleton', () => {
  beforeEach(async () => {
    vi.resetModules()
  ...
  it('requireProvider throws when nothing is bonded', () => {
    expect(() => requireProvider()).toThrow(/AISummarization provider not configured/)
  })
