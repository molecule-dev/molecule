...
Generated: 2026-09-24T12:32:51.050Z
-->
...
```typescript
function getProvider(name?: string): SandboxProvider | null
```

- `name` — Optional named-provider slot; omitted reads the singleton.
...
- `name` — Optional named-provider slot; omitted checks the singleton.
...
```typescript
function requireProvider(name?: string): SandboxProvider
```
...
```typescript
function setProvider(provider: SandboxProvider, name?: string): void
```

- `provider` — The sandbox provider implementation to bond.
- `name` — Optional named-provider slot (e.g. `'production'`).
