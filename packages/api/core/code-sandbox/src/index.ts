 ...
 * - **`exec`'s `timeout` is MILLISECONDS** (`30_000`, not `30`), and paths passed to
 *   `writeFile`/`readFile` are resolved inside the sandbox — they never touch the host.
 * - **Always `destroy(sandbox.id)` a sandbox you are done with** (a `finally` block) —
 *   providers bill/hold resources until then; `stop()`/`sleep()` keep it around.
 *
 * @example
 * ```typescript
 * import { requireProvider, setProvider } from '@molecule/api-code-sandbox'
 * // E2B needs E2B_API_KEY; swap to `@molecule/api-code-sandbox-docker` for a local Docker daemon.
 * import { createProvider } from '@molecule/api-code-sandbox-e2b'
 *
 * // Startup: bond exactly one provider before any sandbox call.
 * setProvider(createProvider({ apiKey: process.env.E2B_API_KEY, defaultPreviewPort: 5173 }))
 *
 ...
 *   console.log(sandbox.getPreviewUrl(5173)) // proxy/iframe target for a dev server on :5173
 * } finally {
