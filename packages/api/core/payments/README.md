...
Generated: 2026-09-04T17:16:33.718Z
-->
...
#### `resolveBillingPortalReturnUrl(returnPath)`
...
- `returnPath` — Optional app-relative path to return to (e.g. `/billing`). Falls back to `PAYMENTS_BILLING_RETURN_PATH`, then the app root.

**Returns:** The absolute return URL.

#### `resolveCheckoutRedirectUrls(options)`
...
top-level redirect from the provider's domain to a _different_ API host
...
- `options` — The provider name and its session-id placeholder.

**Returns:** The success/cancel URLs, the app origin, and whether a localhost fallback origin had to be used.
...
| RevenueCat  | `@molecule/api-payments-revenuecat` |
