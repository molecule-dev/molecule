...
import type { CodeSandboxTranslations } from '../types.js'

const reference = locales.en as CodeSandboxTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof CodeSandboxTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, CodeSandboxTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('code-sandbox locale bond', () => {
