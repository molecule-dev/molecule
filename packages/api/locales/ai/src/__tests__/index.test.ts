...
const reference = locales.en as AiTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof AiTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, AiTranslations] => entry[1] !== null && typeof entry[1] === 'object',
)

describe('ai locale bond', () => {
