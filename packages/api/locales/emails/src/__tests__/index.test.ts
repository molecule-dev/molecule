...
import type { EmailsTranslations } from '../types.js'

const reference = locales.en as EmailsTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof EmailsTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, EmailsTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('emails locale bond', () => {
