...
const reference = locales.en as QueueTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof QueueTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, QueueTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('queue locale bond', () => {
