...
const reference = locales.en as AnalyticsTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof AnalyticsTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, AnalyticsTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('analytics locale bond', () => {
