...
const reference = locales.en as MonitoringTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof MonitoringTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, MonitoringTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('monitoring locale bond', () => {
