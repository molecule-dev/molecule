...
const reference = locales.en as SecretsTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof SecretsTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, SecretsTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('secrets locale bond', () => {
