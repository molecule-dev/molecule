...
const reference = locales.en as ProductTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof ProductTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, ProductTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('product locale bond', () => {
