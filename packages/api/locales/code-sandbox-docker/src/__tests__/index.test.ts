...
const reference = locales.en as CodeSandboxDockerTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof CodeSandboxDockerTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, CodeSandboxDockerTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('code-sandbox-docker locale bond', () => {
