import type { CodeSandboxDockerTranslations } from './types.js'

/** Code Sandbox Docker translations for Indonesian. */
export const id: CodeSandboxDockerTranslations = {
  'codeSandbox.docker.error.readFailed': 'Gagal membaca {{path}}: {{error}}',
  'codeSandbox.docker.error.readDirFailed': 'Gagal menampilkan isi {{path}}: {{error}}',
  'codeSandbox.docker.error.writeFailed': 'Gagal menulis {{path}}: {{error}}',
  'codeSandbox.docker.error.deleteFailed': 'Gagal menghapus {{path}}: {{error}}',
  'codeSandbox.docker.error.apiError': 'Docker API {{method}} {{path}}: {{status}} {{error}}',
}
