import type { CodeSandboxDockerTranslations } from './types.js'

/** Code Sandbox Docker translations for Amharic. */
export const am: CodeSandboxDockerTranslations = {
  'codeSandbox.docker.error.readFailed': '{{path}} ማንበብ አልተሳካም: {{error}}',
  'codeSandbox.docker.error.readDirFailed': '{{path}}ን መዘርዘር አልተሳካም፦ {{error}}',
  'codeSandbox.docker.error.writeFailed': '{{path}} መጻፍ አልተሳካም: {{error}}',
  'codeSandbox.docker.error.deleteFailed': '{{path}} መሰረዝ አልተሳካም: {{error}}',
  'codeSandbox.docker.error.apiError': 'Docker API {{method}} {{path}}: {{status}} {{error}}',
}
