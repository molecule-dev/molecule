import type { CodeSandboxDockerTranslations } from './types.js'

/** Code Sandbox Docker translations for Hebrew. */
export const he: CodeSandboxDockerTranslations = {
  'codeSandbox.docker.error.readFailed': 'נכשל בקריאת {{path}}: {{error}}',
  'codeSandbox.docker.error.readDirFailed': 'הצגת התוכן של {{path}} נכשלה: {{error}}',
  'codeSandbox.docker.error.writeFailed': 'נכשל בכתיבת {{path}}: {{error}}',
  'codeSandbox.docker.error.deleteFailed': 'נכשל במחיקת {{path}}: {{error}}',
  'codeSandbox.docker.error.apiError': 'Docker API {{method}} {{path}}: {{status}} {{error}}',
}
