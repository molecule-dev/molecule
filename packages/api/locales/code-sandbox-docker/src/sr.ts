import type { CodeSandboxDockerTranslations } from './types.js'

/** Code Sandbox Docker translations for Serbian. */
export const sr: CodeSandboxDockerTranslations = {
  'codeSandbox.docker.error.readFailed': 'Неуспело читање {{path}}: {{error}}',
  'codeSandbox.docker.error.readDirFailed': 'Неуспешно излиставање {{path}}: {{error}}',
  'codeSandbox.docker.error.writeFailed': 'Неуспело писање {{path}}: {{error}}',
  'codeSandbox.docker.error.deleteFailed': 'Неуспело брисање {{path}}: {{error}}',
  'codeSandbox.docker.error.apiError': 'Docker API {{method}} {{path}}: {{status}} {{error}}',
}
