import type { CodeSandboxDockerTranslations } from './types.js'

/** Code Sandbox Docker translations for Bengali. */
export const bn: CodeSandboxDockerTranslations = {
  'codeSandbox.docker.error.readFailed': '{{path}} পড়তে ব্যর্থ: {{error}}',
  'codeSandbox.docker.error.readDirFailed': '{{path}} তালিকাভুক্ত করতে ব্যর্থ: {{error}}',
  'codeSandbox.docker.error.writeFailed': '{{path}} লিখতে ব্যর্থ: {{error}}',
  'codeSandbox.docker.error.deleteFailed': '{{path}} মুছতে ব্যর্থ: {{error}}',
  'codeSandbox.docker.error.apiError': 'Docker API {{method}} {{path}}: {{status}} {{error}}',
}
