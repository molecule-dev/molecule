import type { CodeSandboxDockerTranslations } from './types.js'

/** Code Sandbox Docker translations for Sinhala. */
export const si: CodeSandboxDockerTranslations = {
  'codeSandbox.docker.error.readFailed': '{{path}} කියවීමට අසමත් විය: {{error}}',
  'codeSandbox.docker.error.readDirFailed': '{{path}} ලැයිස්තුගත කිරීමට අසමත් විය: {{error}}',
  'codeSandbox.docker.error.writeFailed': '{{path}} ලිවීමට අසමත් විය: {{error}}',
  'codeSandbox.docker.error.deleteFailed': '{{path}} මකා දැමීමට අසමත් විය: {{error}}',
  'codeSandbox.docker.error.apiError': 'Docker API {{method}} {{path}}: {{status}} {{error}}',
}
