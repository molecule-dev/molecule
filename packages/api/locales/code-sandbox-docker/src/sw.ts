import type { CodeSandboxDockerTranslations } from './types.js'

/** Code Sandbox Docker translations for Swahili. */
export const sw: CodeSandboxDockerTranslations = {
  'codeSandbox.docker.error.readFailed': 'Imeshindwa kusoma {{path}}: {{error}}',
  'codeSandbox.docker.error.readDirFailed': 'Imeshindwa kuorodhesha {{path}}: {{error}}',
  'codeSandbox.docker.error.writeFailed': 'Imeshindwa kuandika {{path}}: {{error}}',
  'codeSandbox.docker.error.deleteFailed': 'Imeshindwa kufuta {{path}}: {{error}}',
  'codeSandbox.docker.error.apiError': 'Docker API {{method}} {{path}}: {{status}} {{error}}',
}
