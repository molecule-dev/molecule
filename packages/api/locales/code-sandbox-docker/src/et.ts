import type { CodeSandboxDockerTranslations } from './types.js'

/** Code Sandbox Docker translations for Estonian. */
export const et: CodeSandboxDockerTranslations = {
  'codeSandbox.docker.error.readFailed': '{{path}} lugemine ebaõnnestus: {{error}}',
  'codeSandbox.docker.error.readDirFailed': '{{path}} loendamine ebaõnnestus: {{error}}',
  'codeSandbox.docker.error.writeFailed': '{{path}} kirjutamine ebaõnnestus: {{error}}',
  'codeSandbox.docker.error.deleteFailed': '{{path}} kustutamine ebaõnnestus: {{error}}',
  'codeSandbox.docker.error.apiError': 'Docker API {{method}} {{path}}: {{status}} {{error}}',
}
