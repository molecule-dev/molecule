import type { CodeSandboxDockerTranslations } from './types.js'

/** Code Sandbox Docker translations for Galician. */
export const gl: CodeSandboxDockerTranslations = {
  'codeSandbox.docker.error.readFailed': 'Erro ao ler {{path}}: {{error}}',
  'codeSandbox.docker.error.readDirFailed': 'Non se puido listar {{path}}: {{error}}',
  'codeSandbox.docker.error.writeFailed': 'Erro ao escribir {{path}}: {{error}}',
  'codeSandbox.docker.error.deleteFailed': 'Erro ao eliminar {{path}}: {{error}}',
  'codeSandbox.docker.error.apiError': 'Docker API {{method}} {{path}}: {{status}} {{error}}',
}
