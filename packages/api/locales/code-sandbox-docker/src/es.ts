import type { CodeSandboxDockerTranslations } from './types.js'

/** Code Sandbox Docker translations for Spanish. */
export const es: CodeSandboxDockerTranslations = {
  'codeSandbox.docker.error.readFailed': 'Error al leer {{path}}: {{error}}',
  'codeSandbox.docker.error.readDirFailed': 'Error al listar {{path}}: {{error}}',
  'codeSandbox.docker.error.writeFailed': 'Error al escribir {{path}}: {{error}}',
  'codeSandbox.docker.error.deleteFailed': 'Error al eliminar {{path}}: {{error}}',
  'codeSandbox.docker.error.apiError': 'Docker API {{method}} {{path}}: {{status}} {{error}}',
}
