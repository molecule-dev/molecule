import type { CodeSandboxDockerTranslations } from './types.js'

/** Code Sandbox Docker translations for Albanian. */
export const sq: CodeSandboxDockerTranslations = {
  'codeSandbox.docker.error.readFailed': 'Dështoi leximi i {{path}}: {{error}}',
  'codeSandbox.docker.error.readDirFailed': 'Dështoi listimi i {{path}}: {{error}}',
  'codeSandbox.docker.error.writeFailed': 'Dështoi shkrimi i {{path}}: {{error}}',
  'codeSandbox.docker.error.deleteFailed': 'Dështoi fshirja e {{path}}: {{error}}',
  'codeSandbox.docker.error.apiError': 'Docker API {{method}} {{path}}: {{status}} {{error}}',
}
