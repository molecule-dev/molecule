import type { CodeSandboxDockerTranslations } from './types.js'

/** Code Sandbox Docker translations for Vietnamese. */
export const vi: CodeSandboxDockerTranslations = {
  'codeSandbox.docker.error.readFailed': 'Không thể đọc {{path}}: {{error}}',
  'codeSandbox.docker.error.readDirFailed': 'Không thể liệt kê {{path}}: {{error}}',
  'codeSandbox.docker.error.writeFailed': 'Không thể ghi {{path}}: {{error}}',
  'codeSandbox.docker.error.deleteFailed': 'Không thể xóa {{path}}: {{error}}',
  'codeSandbox.docker.error.apiError': 'Docker API {{method}} {{path}}: {{status}} {{error}}',
}
