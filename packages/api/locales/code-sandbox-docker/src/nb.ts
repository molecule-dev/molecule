import type { CodeSandboxDockerTranslations } from './types.js'

/** Code Sandbox Docker translations for Norwegian Bokmål. */
export const nb: CodeSandboxDockerTranslations = {
  'codeSandbox.docker.error.readFailed': 'Kunne ikke lese {{path}}: {{error}}',
  'codeSandbox.docker.error.readDirFailed': 'Kunne ikke liste {{path}}: {{error}}',
  'codeSandbox.docker.error.writeFailed': 'Kunne ikke skrive {{path}}: {{error}}',
  'codeSandbox.docker.error.deleteFailed': 'Kunne ikke slette {{path}}: {{error}}',
  'codeSandbox.docker.error.apiError': 'Docker API {{method}} {{path}}: {{status}} {{error}}',
}
