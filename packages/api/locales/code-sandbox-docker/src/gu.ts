import type { CodeSandboxDockerTranslations } from './types.js'

/** Code Sandbox Docker translations for Gujarati. */
export const gu: CodeSandboxDockerTranslations = {
  'codeSandbox.docker.error.readFailed': '{{path}} વાંચવામાં નિષ્ફળ: {{error}}',
  'codeSandbox.docker.error.readDirFailed': '{{path}} ની યાદી બનાવવામાં નિષ્ફળ: {{error}}',
  'codeSandbox.docker.error.writeFailed': '{{path}} લખવામાં નિષ્ફળ: {{error}}',
  'codeSandbox.docker.error.deleteFailed': '{{path}} કાઢવામાં નિષ્ફળ: {{error}}',
  'codeSandbox.docker.error.apiError': 'Docker API {{method}} {{path}}: {{status}} {{error}}',
}
