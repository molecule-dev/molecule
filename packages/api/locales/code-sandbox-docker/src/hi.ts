import type { CodeSandboxDockerTranslations } from './types.js'

/** Code Sandbox Docker translations for Hindi. */
export const hi: CodeSandboxDockerTranslations = {
  'codeSandbox.docker.error.readFailed': '{{path}} पढ़ने में विफल: {{error}}',
  'codeSandbox.docker.error.readDirFailed': '{{path}} को सूचीबद्ध करने में विफल: {{error}}',
  'codeSandbox.docker.error.writeFailed': '{{path}} लिखने में विफल: {{error}}',
  'codeSandbox.docker.error.deleteFailed': '{{path}} हटाने में विफल: {{error}}',
  'codeSandbox.docker.error.apiError': 'Docker API {{method}} {{path}}: {{status}} {{error}}',
}
