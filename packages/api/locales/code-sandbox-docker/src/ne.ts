import type { CodeSandboxDockerTranslations } from './types.js'

/** Code Sandbox Docker translations for Nepali. */
export const ne: CodeSandboxDockerTranslations = {
  'codeSandbox.docker.error.readFailed': '{{path}} पढ्न असफल: {{error}}',
  'codeSandbox.docker.error.readDirFailed': '{{path}} सूचीबद्ध गर्न असफल: {{error}}',
  'codeSandbox.docker.error.writeFailed': '{{path}} लेख्न असफल: {{error}}',
  'codeSandbox.docker.error.deleteFailed': '{{path}} मेट्न असफल: {{error}}',
  'codeSandbox.docker.error.apiError': 'Docker API {{method}} {{path}}: {{status}} {{error}}',
}
