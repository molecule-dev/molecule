import type { CodeSandboxDockerTranslations } from './types.js'

/** Code Sandbox Docker translations for Afrikaans. */
export const af: CodeSandboxDockerTranslations = {
  'codeSandbox.docker.error.readFailed': 'Kon nie {{path}} lees nie: {{error}}',
  'codeSandbox.docker.error.readDirFailed': 'Kon nie {{path}} lys nie: {{error}}',
  'codeSandbox.docker.error.writeFailed': 'Kon nie {{path}} skryf nie: {{error}}',
  'codeSandbox.docker.error.deleteFailed': 'Kon nie {{path}} uitvee nie: {{error}}',
  'codeSandbox.docker.error.apiError': 'Docker API {{method}} {{path}}: {{status}} {{error}}',
}
