import type { CodeSandboxDockerTranslations } from './types.js'

/** Code Sandbox Docker translations for Thai. */
export const th: CodeSandboxDockerTranslations = {
  'codeSandbox.docker.error.readFailed': 'การอ่าน {{path}} ล้มเหลว: {{error}}',
  'codeSandbox.docker.error.readDirFailed': 'ไม่สามารถแสดงรายการ {{path}}: {{error}}',
  'codeSandbox.docker.error.writeFailed': 'การเขียน {{path}} ล้มเหลว: {{error}}',
  'codeSandbox.docker.error.deleteFailed': 'การลบ {{path}} ล้มเหลว: {{error}}',
  'codeSandbox.docker.error.apiError': 'Docker API {{method}} {{path}}: {{status}} {{error}}',
}
