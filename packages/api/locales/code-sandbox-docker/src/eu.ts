import type { CodeSandboxDockerTranslations } from './types.js'

/** Code Sandbox Docker translations for Basque. */
export const eu: CodeSandboxDockerTranslations = {
  'codeSandbox.docker.error.readFailed': 'Huts egin du {{path}} irakurtzean: {{error}}',
  'codeSandbox.docker.error.readDirFailed': 'Ezin izan da {{path}} zerrendatu: {{error}}',
  'codeSandbox.docker.error.writeFailed': 'Huts egin du {{path}} idaztean: {{error}}',
  'codeSandbox.docker.error.deleteFailed': 'Huts egin du {{path}} ezabatzean: {{error}}',
  'codeSandbox.docker.error.apiError': 'Docker API {{method}} {{path}}: {{status}} {{error}}',
}
