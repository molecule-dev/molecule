import type { CodeSandboxDockerTranslations } from './types.js'

/** Code Sandbox Docker translations for Urdu. */
export const ur: CodeSandboxDockerTranslations = {
  'codeSandbox.docker.error.readFailed': '{{path}} پڑھنے میں ناکام: {{error}}',
  'codeSandbox.docker.error.readDirFailed': '{{path}} کی فہرست بنانے میں ناکام: {{error}}',
  'codeSandbox.docker.error.writeFailed': '{{path}} لکھنے میں ناکام: {{error}}',
  'codeSandbox.docker.error.deleteFailed': '{{path}} حذف کرنے میں ناکام: {{error}}',
  'codeSandbox.docker.error.apiError': 'Docker API {{method}} {{path}}: {{status}} {{error}}',
}
