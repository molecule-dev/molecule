import type { CodeSandboxDockerTranslations } from './types.js'

/** Code Sandbox Docker translations for Latvian. */
export const lv: CodeSandboxDockerTranslations = {
  'codeSandbox.docker.error.readFailed': 'Neizdevās nolasīt {{path}}: {{error}}',
  'codeSandbox.docker.error.readDirFailed': 'Neizdevās uzskaitīt {{path}}: {{error}}',
  'codeSandbox.docker.error.writeFailed': 'Neizdevās ierakstīt {{path}}: {{error}}',
  'codeSandbox.docker.error.deleteFailed': 'Neizdevās dzēst {{path}}: {{error}}',
  'codeSandbox.docker.error.apiError': 'Docker API {{method}} {{path}}: {{status}} {{error}}',
}
