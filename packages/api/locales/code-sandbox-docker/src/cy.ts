import type { CodeSandboxDockerTranslations } from './types.js'

/** Code Sandbox Docker translations for Welsh. */
export const cy: CodeSandboxDockerTranslations = {
  'codeSandbox.docker.error.readFailed': 'Methodd darllen {{path}}: {{error}}',
  'codeSandbox.docker.error.readDirFailed': 'Methwyd rhestru {{path}}: {{error}}',
  'codeSandbox.docker.error.writeFailed': 'Methodd ysgrifennu {{path}}: {{error}}',
  'codeSandbox.docker.error.deleteFailed': 'Methodd dileu {{path}}: {{error}}',
  'codeSandbox.docker.error.apiError': 'Docker API {{method}} {{path}}: {{status}} {{error}}',
}
