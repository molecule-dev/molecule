import type { CodeSandboxDockerTranslations } from './types.js'

/** Code Sandbox Docker translations for Russian. */
export const ru: CodeSandboxDockerTranslations = {
  'codeSandbox.docker.error.readFailed': 'Не удалось прочитать {{path}}: {{error}}',
  'codeSandbox.docker.error.readDirFailed': 'Не удалось получить список {{path}}: {{error}}',
  'codeSandbox.docker.error.writeFailed': 'Не удалось записать {{path}}: {{error}}',
  'codeSandbox.docker.error.deleteFailed': 'Не удалось удалить {{path}}: {{error}}',
  'codeSandbox.docker.error.apiError': 'Docker API {{method}} {{path}}: {{status}} {{error}}',
}
