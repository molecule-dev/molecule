import type { CodeSandboxDockerTranslations } from './types.js'

/** Code Sandbox Docker translations for Lithuanian. */
export const lt: CodeSandboxDockerTranslations = {
  'codeSandbox.docker.error.readFailed': 'Nepavyko perskaityti {{path}}: {{error}}',
  'codeSandbox.docker.error.readDirFailed': 'Nepavyko išvardyti {{path}}: {{error}}',
  'codeSandbox.docker.error.writeFailed': 'Nepavyko įrašyti {{path}}: {{error}}',
  'codeSandbox.docker.error.deleteFailed': 'Nepavyko ištrinti {{path}}: {{error}}',
  'codeSandbox.docker.error.apiError': 'Docker API {{method}} {{path}}: {{status}} {{error}}',
}
