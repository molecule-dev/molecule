import type { CodeSandboxDockerTranslations } from './types.js'

/** Code Sandbox Docker translations for Kazakh. */
export const kk: CodeSandboxDockerTranslations = {
  'codeSandbox.docker.error.readFailed': '{{path}} оқу сәтсіз аяқталды: {{error}}',
  'codeSandbox.docker.error.readDirFailed': '{{path}} тізімін алу сәтсіз аяқталды: {{error}}',
  'codeSandbox.docker.error.writeFailed': '{{path}} жазу сәтсіз аяқталды: {{error}}',
  'codeSandbox.docker.error.deleteFailed': '{{path}} жою сәтсіз аяқталды: {{error}}',
  'codeSandbox.docker.error.apiError': 'Docker API {{method}} {{path}}: {{status}} {{error}}',
}
