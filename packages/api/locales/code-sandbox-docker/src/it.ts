import type { CodeSandboxDockerTranslations } from './types.js'

/** Code Sandbox Docker translations for Italian. */
export const it: CodeSandboxDockerTranslations = {
  'codeSandbox.docker.error.readFailed': 'Impossibile leggere {{path}}: {{error}}',
  'codeSandbox.docker.error.readDirFailed': 'Impossibile elencare {{path}}: {{error}}',
  'codeSandbox.docker.error.writeFailed': 'Impossibile scrivere {{path}}: {{error}}',
  'codeSandbox.docker.error.deleteFailed': 'Impossibile eliminare {{path}}: {{error}}',
  'codeSandbox.docker.error.apiError': 'Docker API {{method}} {{path}}: {{status}} {{error}}',
}
