import type { CodeSandboxDockerTranslations } from './types.js'

/** Code Sandbox Docker translations for Ukrainian. */
export const uk: CodeSandboxDockerTranslations = {
  'codeSandbox.docker.error.readFailed': 'Не вдалося прочитати {{path}}: {{error}}',
  'codeSandbox.docker.error.readDirFailed': 'Не вдалося отримати список {{path}}: {{error}}',
  'codeSandbox.docker.error.writeFailed': 'Не вдалося записати {{path}}: {{error}}',
  'codeSandbox.docker.error.deleteFailed': 'Не вдалося видалити {{path}}: {{error}}',
  'codeSandbox.docker.error.apiError': 'Docker API {{method}} {{path}}: {{status}} {{error}}',
}
