import type { CodeSandboxDockerTranslations } from './types.js'

/** Code Sandbox Docker translations for Romanian. */
export const ro: CodeSandboxDockerTranslations = {
  'codeSandbox.docker.error.readFailed': 'Citirea {{path}} a eșuat: {{error}}',
  'codeSandbox.docker.error.readDirFailed': 'Nu s-a putut lista {{path}}: {{error}}',
  'codeSandbox.docker.error.writeFailed': 'Scrierea {{path}} a eșuat: {{error}}',
  'codeSandbox.docker.error.deleteFailed': 'Ștergerea {{path}} a eșuat: {{error}}',
  'codeSandbox.docker.error.apiError': 'Docker API {{method}} {{path}}: {{status}} {{error}}',
}
