import type { CodeSandboxDockerTranslations } from './types.js'

/** Code Sandbox Docker translations for Portuguese. */
export const pt: CodeSandboxDockerTranslations = {
  'codeSandbox.docker.error.readFailed': 'Falha ao ler {{path}}: {{error}}',
  'codeSandbox.docker.error.readDirFailed': 'Falha ao listar {{path}}: {{error}}',
  'codeSandbox.docker.error.writeFailed': 'Falha ao escrever {{path}}: {{error}}',
  'codeSandbox.docker.error.deleteFailed': 'Falha ao excluir {{path}}: {{error}}',
  'codeSandbox.docker.error.apiError': 'Docker API {{method}} {{path}}: {{status}} {{error}}',
}
