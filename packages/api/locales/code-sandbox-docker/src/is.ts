import type { CodeSandboxDockerTranslations } from './types.js'

/** Code Sandbox Docker translations for Icelandic. */
export const is: CodeSandboxDockerTranslations = {
  'codeSandbox.docker.error.readFailed': 'Mistókst að lesa {{path}}: {{error}}',
  'codeSandbox.docker.error.readDirFailed': 'Ekki tókst að lista {{path}}: {{error}}',
  'codeSandbox.docker.error.writeFailed': 'Mistókst að skrifa {{path}}: {{error}}',
  'codeSandbox.docker.error.deleteFailed': 'Mistókst að eyða {{path}}: {{error}}',
  'codeSandbox.docker.error.apiError': 'Docker API {{method}} {{path}}: {{status}} {{error}}',
}
