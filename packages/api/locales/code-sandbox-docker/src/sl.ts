import type { CodeSandboxDockerTranslations } from './types.js'

/** Code Sandbox Docker translations for Slovenian. */
export const sl: CodeSandboxDockerTranslations = {
  'codeSandbox.docker.error.readFailed': 'Branje {{path}} ni uspelo: {{error}}',
  'codeSandbox.docker.error.readDirFailed': 'Izpis {{path}} ni uspel: {{error}}',
  'codeSandbox.docker.error.writeFailed': 'Pisanje {{path}} ni uspelo: {{error}}',
  'codeSandbox.docker.error.deleteFailed': 'Brisanje {{path}} ni uspelo: {{error}}',
  'codeSandbox.docker.error.apiError': 'Docker API {{method}} {{path}}: {{status}} {{error}}',
}
