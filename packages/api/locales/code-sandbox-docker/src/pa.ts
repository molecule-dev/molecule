import type { CodeSandboxDockerTranslations } from './types.js'

/** Code Sandbox Docker translations for Punjabi. */
export const pa: CodeSandboxDockerTranslations = {
  'codeSandbox.docker.error.readFailed': '{{path}} ਪੜ੍ਹਨ ਵਿੱਚ ਅਸਫਲ: {{error}}',
  'codeSandbox.docker.error.readDirFailed': '{{path}} ਦੀ ਸੂਚੀ ਬਣਾਉਣ ਵਿੱਚ ਅਸਫਲ: {{error}}',
  'codeSandbox.docker.error.writeFailed': '{{path}} ਲਿਖਣ ਵਿੱਚ ਅਸਫਲ: {{error}}',
  'codeSandbox.docker.error.deleteFailed': '{{path}} ਮਿਟਾਉਣ ਵਿੱਚ ਅਸਫਲ: {{error}}',
  'codeSandbox.docker.error.apiError': 'Docker API {{method}} {{path}}: {{status}} {{error}}',
}
