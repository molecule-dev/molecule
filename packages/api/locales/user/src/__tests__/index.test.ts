...
const reference = locales.en as UserTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof UserTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, UserTranslations] => entry[1] !== null && typeof entry[1] === 'object',
)

describe('user locale bond', () => {
