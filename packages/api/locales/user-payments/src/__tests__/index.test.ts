...
const reference = locales.en as UserPaymentTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof UserPaymentTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, UserPaymentTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('user-payments locale bond', () => {
