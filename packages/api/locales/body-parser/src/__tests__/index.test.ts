...
const reference = locales.en as BodyParserTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof BodyParserTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, BodyParserTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('body-parser locale bond', () => {
