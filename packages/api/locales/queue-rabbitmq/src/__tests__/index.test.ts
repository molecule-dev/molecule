...
import type { QueueRabbitmqTranslations } from '../types.js'

const reference = locales.en as QueueRabbitmqTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof QueueRabbitmqTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, QueueRabbitmqTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('queue-rabbitmq locale bond', () => {
