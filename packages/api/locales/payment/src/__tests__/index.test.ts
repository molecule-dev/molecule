...
import type { PaymentTranslations } from '../types.js'

const reference = locales.en as PaymentTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof PaymentTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, PaymentTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('payment locale bond', () => {
