...
import type { DeviceTranslations } from '../types.js'

const reference = locales.en as DeviceTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof DeviceTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, DeviceTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('device locale bond', () => {
