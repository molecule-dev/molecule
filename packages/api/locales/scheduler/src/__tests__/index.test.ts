...
import type { SchedulerTranslations } from '../types.js'

const reference = locales.en as SchedulerTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof SchedulerTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, SchedulerTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('scheduler locale bond', () => {
