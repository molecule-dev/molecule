...
const reference = locales.en as DatabaseTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof DatabaseTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, DatabaseTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('database locale bond', () => {
