...
import type { SecretsDopplerTranslations } from '../types.js'

const reference = locales.en as SecretsDopplerTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof SecretsDopplerTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, SecretsDopplerTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('secrets-doppler locale bond', () => {
