...
const reference = locales.en as MessageTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof MessageTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, MessageTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('resource-message locale bond', () => {
