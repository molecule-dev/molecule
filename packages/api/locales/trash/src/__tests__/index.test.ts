...
const reference = locales.en as TrashTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof TrashTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, TrashTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('trash locale bond', () => {
