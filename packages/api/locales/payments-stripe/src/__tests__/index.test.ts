...
const reference = locales.en as PaymentsStripeTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof PaymentsStripeTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, PaymentsStripeTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('payments-stripe locale bond', () => {
