...
const reference = locales.en as ConfigTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof ConfigTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, ConfigTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('config locale bond', () => {
