...
const reference = locales.en as StatusTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof StatusTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, StatusTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('status-page locale bond', () => {
