...
const reference = locales.en as NotificationsTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof NotificationsTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, NotificationsTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('notifications locale bond', () => {
