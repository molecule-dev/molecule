...
const reference = locales.en as PropertyTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof PropertyTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, PropertyTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('property locale bond', () => {
