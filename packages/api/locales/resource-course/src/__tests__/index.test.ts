...
const reference = locales.en as ResourceCourseTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof ResourceCourseTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, ResourceCourseTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('resource-course locale bond', () => {
