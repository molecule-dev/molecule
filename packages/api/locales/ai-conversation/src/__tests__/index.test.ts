...
import type { ConversationTranslations } from '../types.js'

const reference = locales.en as ConversationTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof ConversationTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, ConversationTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('ai-conversation locale bond', () => {
