...
const reference = locales.en as VersionHistoryTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof VersionHistoryTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, VersionHistoryTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('resource-version-history locale bond', () => {
