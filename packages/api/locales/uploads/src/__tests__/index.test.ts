...
const reference = locales.en as UploadsTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof UploadsTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, UploadsTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('uploads locale bond', () => {
