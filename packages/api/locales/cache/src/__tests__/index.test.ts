...
const reference = locales.en as CacheTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof CacheTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, CacheTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('cache locale bond', () => {
