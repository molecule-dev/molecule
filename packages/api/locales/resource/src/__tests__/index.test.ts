...
import type { ResourceTranslations } from '../types.js'

const reference = locales.en as ResourceTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof ResourceTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, ResourceTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('resource locale bond', () => {
