...
Generated: 2026-08-14T10:30:01.448Z
-->
...
#### `assertSafeImageFormat(input)`

Throw when an image is in a format whose measurement parser is vulnerable.

```typescript
function assertSafeImageFormat(input: { bytes?: Uint8Array; mimeType?: string; src?: string }): void
```

- `input` — What is known about the image: raw bytes when available, plus any declared MIME type and source path/URL.

#### `blockedFormatFromLabel(label)`

Blocked format implied by a MIME type or file extension, if any.

```typescript
function blockedFormatFromLabel(label: string | undefined): 'icns' | 'jxl' | 'heif' | null
```
...
```typescript
function sniffBlockedFormat(bytes: Uint8Array<ArrayBufferLike>): 'icns' | 'jxl' | 'heif' | null
```

- `bytes` — The start of the image payload (64 bytes is ample).
