...
describe('isPrivateHost (oembed)', () => {
  ...
  it('allows public hosts', () => {
    ...
    expect(isPrivateHost('www.youtube.com')).toBe(false)
...
describe('isIpLiteral', () => {
  ...
  it('rejects hostnames', () => {
    expect(isIpLiteral('www.youtube.com')).toBe(false)
...
describe('isHostBlocked (literal fast-path + DNS)', () => {
  ...
  it('resolves a hostname and allows it when it points at a public address', async () => {
    expect(await isHostBlocked('www.youtube.com', publicLookup)).toBe(false)
  })
})
