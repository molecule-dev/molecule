 ...
 * MySQL client tools (`mysqldump`, `mysqladmin` — not installed on CI): tiny
...
import { createMysqlExternalStateProvider } from '../index.js'
...
describe('README @example', () => {
  ...
  beforeAll(async () => {
    root = await mkdtemp(join(tmpdir(), 'mysql-external-state-readme-'))
    process.chdir(root)
    const bin = join(root, 'bin')
    await mkdir(bin)
    // mysqldump stand-in: fails when asked for "app_broken", otherwise emits a dump
    // naming its argv and MYSQL_PWD (proving the password stayed out of argv).
    await writeFile(
      join(bin, 'mysqldump'),
      '#!/bin/sh\n' +
        'case "$*" in *app_broken*) echo "Access denied" >&2; exit 2;; esac\n' +
        'printf "%s pw=%s args=%s" "-- dump" "$MYSQL_PWD" "$*"\n',
    )
    await writeFile(
      join(bin, 'mysqladmin'),
      `#!/bin/sh\necho "$*" >> ${join(root, 'dropped.log')}\n`,
    )
    await chmod(join(bin, 'mysqldump'), 0o755)
    await chmod(join(bin, 'mysqladmin'), 0o755)
    vi.stubEnv('PATH', `${bin}${delimiter}${process.env.PATH ?? ''}`)
    vi.stubEnv('MYSQL_PWD', 'test-password')
  ...
  it('dumps the declared database (with routines/triggers/events), archives it verified, and only then drops it', async () => {
    const { result, records } = await archiveProject('proj_123')

    expect(result.verified).toBe(true)
    expect(archiveIdByProject.get('proj_123')).toBe(result.storageId)
    expect(records).toEqual([{ kind: 'mysql', id: 'app_proj_123', part: expect.any(String) }])
    expect(await readFile(join(root, 'dropped.log'), 'utf8')).toBe(
      '--host=db.example.com --user=archiver --force drop app_proj_123\n',
    )
    ...
    expect(new TextDecoder().decode(dumpPart?.content)).toBe(
      '-- dump pw=test-password args=--host=db.example.com --port=3306 --user=archiver ' +
        '--single-transaction --routines --triggers --events --no-tablespaces -- app_proj_123',
    )
  })

  it('THROWS — and drops nothing — when the dump fails', async () => {
    await rm(join(root, 'dropped.log'), { force: true })
    await expect(archiveProject('broken')).rejects.toThrow(/mysqldump exited 2: Access denied/)
