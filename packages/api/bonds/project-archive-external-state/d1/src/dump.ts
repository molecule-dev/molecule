...
export async function restoreFromFile(
  ...
  return await new Promise<void>((resolve, reject) => {
    ...
    // `wrangler d1 execute` takes `--file <path>` and never reads stdin, so the
    // path is passed in argv by the caller and stdin is closed immediately.
    // Piping the dump in here instead (as the sqlite bond does for `sqlite3`)
    // would leave wrangler waiting on an input it does not consume, and the SQL
    // would never be applied — a restore that reports success having done
    // nothing.
    if (srcPath === '') {
      child.stdin.end()
    } else {
