  ...
  for (const cond of conditions) {
    ...
    switch (cond.operator) {
      ...
      case 'in': {
        ...
        if (arr.length === 0) {
          // `IN ()` is a MySQL syntax error; match Postgres `= ANY('{}')` semantics: no match.
          parts.push('1 = 0')
          break
        }
      ...
      case 'not_in': {
        ...
        if (arr.length === 0) {
          // `NOT IN ()` is a MySQL syntax error; match Postgres `!= ALL('{}')`: everything matches.
          parts.push('1 = 1')
          break
        }
        const placeholders = arr.map(() => '?').join(', ')
      ...
      case 'ilike':
        ...
        // LIKE metacharacters, wrap with `%…%`. Previously the raw value was
        // passed through, so `ilike` with value `foo` compiled to `LIKE 'foo'`
        // (exact match, not contains) and user-typed `%`/`_` acted as wildcards —
        // both diverging silently from the postgres bond. LOWER() both sides so
        // the contract holds on case-sensitive (_bin/_cs) collations too; the
        // backslash escapes rely on MySQL's default LIKE escape character.
...
export function createStore(pool: DatabasePool): DataStore {
  ...
  return {
    ...
    async updateById<T = Record<string, unknown>>(
      ...
      // passing `{}` here.
      if (keys.length === 0) {
        const existing = await readById<T>(table, id)
