...
describe('ai-classification-llm provider', () => {
  beforeEach(() => {
    reset()
  ...
  it('targets a named ai provider when `provider` is passed', async () => {
    bond('ai', 'anthropic', makeAI('{"scores": {"spam": 0.2, "ham": 0.8}}'))
  ...
  it('throws when the named ai provider is not bonded', async () => {
    bondAI('{"scores": {"spam": 0.5, "ham": 0.5}}')

    await expect(
      provider.classify({ text: 'x', labels: ['spam', 'ham'], provider: 'missing' }),
    ).rejects.toThrow(/not bonded/i)
  })
