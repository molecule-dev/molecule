/**
 * Browser guard — @molecule/api-cron-node-cron is SERVER-ONLY.
 *
...
if (g.window !== undefined && g.document !== undefined && !g.process?.versions?.node) {
  throw new Error(
    '@molecule/api-cron-node-cron is SERVER-ONLY: it was bundled into browser/client code. Import it only ' +
