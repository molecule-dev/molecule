/**
 * Browser guard — `@molecule/api-agent-transcript-codex` is SERVER-ONLY.
 *
...
if (g.window !== undefined && g.document !== undefined && !g.process?.versions?.node) {
  throw new Error(
    '@molecule/api-agent-transcript-codex is SERVER-ONLY: it was bundled into browser/client code. Import it only ' +
