 ...
 *   getAuthorizeUrl })`** — there is no `setProvider`. The richer
 *   `provider` / `createMicrosoftProvider({ tenantId, clientId, clientSecret })`
 *   object (tokens, refresh, Graph, ID-token verification) is separate and is
 *   NOT what the user-login handlers call.
 * - **`verify()` ALWAYS returns `emailVerified: false`.** The email comes from
 *   Graph `mail` / `userPrincipalName`, which Microsoft does not guarantee is
 *   verified — never auto-link an existing local account on it.
 * - `verify()` returns `null` only for `invalid_grant`; other failures throw.
 *   Default scope is `openid email profile User.Read`.
 *
 ...
 * import { getAuthorizeUrl, serverName, verify } from '@molecule/api-oauth-microsoft'
 *
 * // Startup: bond NAMED by serverName ('microsoft'). Env: OAUTH_MICROSOFT_CLIENT_ID,
 * // OAUTH_MICROSOFT_CLIENT_SECRET (server only), OAUTH_MICROSOFT_TENANT_ID (default 'common'), APP_ORIGIN.
 ...
 * const redirectUri = process.env.APP_ORIGIN ?? 'https://app.example.com' // a registered Web redirect URI
 * const authorizeUrl = getAuthorizeUrl({ state, codeChallenge, codeChallengeMethod: 'S256', redirectUri })
 * // → 302 the browser to authorizeUrl; null means OAUTH_MICROSOFT_CLIENT_ID is unset.
 *
 * // Callback (after checking the returned state equals the cookie): exchange + Graph /me.
 * const user = await verify('code-from-callback-query', codeVerifier, redirectUri)
 * // user: { username: 'ada@contoso.com@microsoft', email, emailVerified: false, oauthId: Graph id }
 * // null → invalid_grant (respond 403); a throw → network/config failure.
 * console.log(authorizeUrl, user?.email)
 * ```
 *
 * @module
 */
