 ...
 * Quick Start) works as written. Only the network (`fetch` to Twitter, reached
...
describe('README @example', () => {
  ...
  beforeEach(() => {
    process.env = {
      ...originalEnv,
      OAUTH_TWITTER_CLIENT_ID: 'test-client-id',
      OAUTH_TWITTER_CLIENT_SECRET: 'test-client-secret',
  ...
  it('bonds the provider, builds the authorize URL and verifies the callback code', async () => {
    const fetchMock = vi.fn(async (input: string | URL | Request, _init?: RequestInit) => {
      const body =
        String(input) === 'https://api.twitter.com/2/oauth2/token'
          ? { access_token: 'x-test-token', token_type: 'bearer', scope: 'users.read tweet.read' }
          : { data: { id: '2244994945', username: 'ada', name: 'Ada Lovelace' } }
    ...
    expect(get<OAuthProviderConfig>('oauth', 'twitter')).toBe(twitter)
