...
describe('README @example', () => {
  ...
  it('bonds Alibaba (Qwen / DashScope) by name and streams the reply through the core', async () => {
    vi.stubEnv('DASHSCOPE_API_KEY', 'test-key')
    vi.stubEnv('ALIBABA_API_KEY', undefined)
    vi.stubEnv('DASHSCOPE_BASE_URL', undefined)
    vi.stubEnv('DASHSCOPE_COMPLETIONS_PATH', undefined)
    const fetchMock = vi
      .fn()
      .mockResolvedValue(
        sseResponse([
