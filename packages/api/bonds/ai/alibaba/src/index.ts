 ...
 *
 * - **Bond it by NAME: `setProvider('alibaba', createProvider(...))`** from `@molecule/api-ai`.
 ...
 *   `getProviderByName('alibaba')` instead. Importing this package wires nothing by itself.
 * - Default model is `qwen3.8-max` (override with `defaultModel` or per-call `params.model`).
 *
 * @example
 * ```typescript
 * import { requireProvider, setProvider } from '@molecule/api-ai'
 * import { createProvider } from '@molecule/api-ai-alibaba'
 *
 * // Startup (server only): bond by name. The key comes from the server env.
 * setProvider('alibaba', createProvider({ apiKey: process.env.DASHSCOPE_API_KEY }))
 *
