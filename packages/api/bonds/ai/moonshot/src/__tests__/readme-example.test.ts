...
describe('README @example', () => {
  ...
  it('bonds Moonshot (Kimi) by name and streams the reply through the core', async () => {
    vi.stubEnv('MOONSHOT_API_KEY', 'test-key')
    vi.stubEnv('MOONSHOT_BASE_URL', undefined)
    vi.stubEnv('MOONSHOT_COMPLETIONS_PATH', undefined)
    vi.stubEnv('KIMI_REASONING_EFFORT', undefined)
    const fetchMock = vi
      .fn()
      .mockResolvedValue(
        sseResponse([
