...
describe('createProvider', () => {
  ...
  it('throws naming MOONSHOT_API_KEY when no key is configured (config or env)', () => {
    const prev = process.env.MOONSHOT_API_KEY
    delete process.env.MOONSHOT_API_KEY
    try {
      expect(() => createProvider()).toThrow(/MOONSHOT_API_KEY/)
    } finally {
      if (prev === undefined) delete process.env.MOONSHOT_API_KEY
      else process.env.MOONSHOT_API_KEY = prev
    }
...
describe('chat() — response handling', () => {
  ...
  it('streaming: a mid-stream error chunk yields partial text then a real error event and STOPS (no done)', async () => {
    const fetch = globalThis.fetch as ReturnType<typeof vi.fn>
    fetch.mockResolvedValue(
      sseResponse([
    ...
    // …and the error is the terminal event.
    expect((events[events.length - 1] as { type: string }).type).toBe('error')
  })
