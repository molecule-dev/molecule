 ...
 *
 * - **Bond it by NAME: `setProvider('xai', createProvider(...))`** from `@molecule/api-ai`.
 ...
 *   `getProviderByName('xai')` instead. Importing this package wires nothing by itself.
 * - Default model is `grok-4.5` (override with `defaultModel` or per-call `params.model`).
 *
 * @example
 * ```typescript
 * import { requireProvider, setProvider } from '@molecule/api-ai'
 * import { createProvider } from '@molecule/api-ai-xai'
 *
 * // Startup (server only): bond by name. The key comes from the server env.
 * setProvider('xai', createProvider({ apiKey: process.env.XAI_API_KEY }))
 *
