 ...
 *
 * - **Bond it by NAME: `setProvider('deepseek', createProvider(...))`** from `@molecule/api-ai`.
 ...
 *   `getProviderByName('deepseek')` instead. Importing this package wires nothing by itself.
 * - Default model is `deepseek-flash` (override with `defaultModel` or per-call `params.model`).
 *
 * @example
 * ```typescript
 * import { requireProvider, setProvider } from '@molecule/api-ai'
 * import { createProvider } from '@molecule/api-ai-deepseek'
 *
 * // Startup (server only): bond by name. The key comes from the server env.
 * setProvider('deepseek', createProvider({ apiKey: process.env.DEEPSEEK_API_KEY }))
 *
