...
describe('GoogleAIProvider — streaming + request mapping', () => {
  ...
  it('targets the streaming endpoint with alt=sse and the key in the query', async () => {
    ...
    expect(urlOf()).toBe(
      'https://test.api/models/gemini-3.8-flash:streamGenerateContent?alt=sse&key=test-key',
    )
  })
  ...
  it('allows a keyless custom endpoint and omits the key query param', async () => {
    ...
    expect(urlOf()).toBe(
      'https://gemini.local-proxy.test/models/gemini-3.8-flash:streamGenerateContent?alt=sse',
    )
  })
  ...
  it('parses a non-streaming response (:generateContent, key in query)', async () => {
    ...
    expect(urlOf()).toBe('https://test.api/models/gemini-3.8-flash:generateContent?key=test-key')
