...
describe('GoogleAIProvider — streaming + request mapping', () => {
  ...
  it('targets the streaming endpoint with alt=sse and the key in the query', async () => {
    ...
    expect(urlOf()).toBe(
      'https://test.api/models/gemini-3.7-flash:streamGenerateContent?alt=sse&key=test-key',
    )
  })
  ...
  it('parses a non-streaming response (:generateContent, key in query)', async () => {
    ...
    expect(urlOf()).toBe('https://test.api/models/gemini-3.7-flash:generateContent?key=test-key')
  ...
  it('honours the default base URL when none is configured', async () => {
    ...
    try {
      ...
      expect(urlOf()).toBe(
        'https://generativelanguage.googleapis.com/v1beta/models/gemini-3.7-flash:streamGenerateContent?alt=sse&key=test-key',
      )
    } finally {
