...
describe('README @example', () => {
  it('grants a role, gates access by it and resolves a public link', async () => {
    setStore(store)
    ...
    expect(await canAccess('document', 'doc-1', 'commenter', 'user-2')).toBe(true)
    expect(await canAccess('document', 'doc-1', 'viewer', 'user-3')).toBe(false)
    await expect(requireRole('document', 'doc-1', 'editor', 'user-2')).resolves.toBeUndefined()
    await expect(requireRole('document', 'doc-1', 'owner', 'user-2')).rejects.toThrow('forbidden')
    expect((await resolveShareLink(link.slug))?.role).toBe('viewer')

    expect(await canAdministerResource('document', 'doc-1', 'owner-1')).toBe(true)
    expect(await canAdministerResource('document', 'doc-1', 'user-2')).toBe(false)

    await revokeShareLink(link.id)
    expect(await resolveShareLink(link.slug)).toBeNull()
