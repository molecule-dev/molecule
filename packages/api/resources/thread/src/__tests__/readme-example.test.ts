...
import { addMessage, createThread, getMessages, getUnreadCount, markRead } from '../index.js'

describe('README @example', () => {
  it('posts messages, counts the unread thread and clears it on markRead', async () => {
    setStore(store)
    ...
    const last = await addMessage(thread.id, userId, { body: 'Reviewed' })
    expect(last?.body).toBe('Reviewed')
    expect(await addMessage(thread.id, 'user-2', { body: 'Intruder' })).toBeNull()
    ...
    expect(page.data.map((m) => m.body)).toEqual(['Draft is up', 'Reviewed'])
