/**
 * Tests for the API key utilities (hash, generate, mask, constant-time-compare).
 */
...
describe('hashPlaintextToken', () => {
  it('produces a deterministic SHA-256 hex digest for the same input', () => {
    const a = hashPlaintextToken('sk_test_abc123')
    const b = hashPlaintextToken('sk_test_abc123')
    expect(a).toBe(b)
    expect(a).toMatch(/^[0-9a-f]{64}$/)
  })

  it('produces a different digest for different inputs', () => {
    expect(hashPlaintextToken('sk_a')).not.toBe(hashPlaintextToken('sk_b'))
  })
})
...
describe('generatePlaintextToken', () => {
  ...
  it('honours a custom prefix', () => {
    const token = generatePlaintextToken('sk_live_')
...
describe('maskPlaintextToken', () => {
  it('keeps the prefix and last 4 characters', () => {
    const masked = maskPlaintextToken('sk_live_HelloWorldABCD', 'sk_live_')
    expect(masked).toBe(`sk_live_…ABCD`)
    expect(masked.endsWith('ABCD')).toBe(true)
    expect(masked.length).toBeLessThan('sk_live_HelloWorldABCD'.length)
  })

  it('infers the prefix when not provided', () => {
    const masked = maskPlaintextToken('sk_test_HelloWorldXYZW')
  ...
  it('exposes only the last MASKED_TAIL_LENGTH characters', () => {
    const masked = maskPlaintextToken('sk_live_VERYLONGTOKEN1234')
