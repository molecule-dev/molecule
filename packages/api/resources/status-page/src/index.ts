 ...
 * import { createRequestHandlerMap } from '@molecule/api-resource-status-page'
 *
 * // Startup: bond the DataStore once (the postgresql bond reads DATABASE_URL).
 * setStore(store)
 *
 * // The handler map is a FACTORY — build it with api-resource's createRequestHandler, then
 * // mount it (what `mlcl inject` generates from `routes`) AFTER the global auth middleware.
 * const Status = createRequestHandlerMap(createRequestHandler)
 * export const router = express.Router()
 * router.get('/status', Status.getStatus) // public
 * router.get('/status/services', Status.listServices) // public
 * router.get('/status/services/:id', Status.getService) // public
 * router.get('/status/incidents', Status.listIncidents) // public, ?status=&limit=&offset=
 * router.get('/status/uptime', Status.getUptime) // public, ?serviceId=
 ...
 *
 * // Admin (session role 'admin'): POST /status/services { name: 'API', url: 'https://api.example.com/health' }
 * //   → 201 { props: { id, name: 'API', method: 'GET', expectedStatus: 200, enabled: true, ... } }
 * // Anyone: GET /status → 200 { status: 'operational', services: [{ id, name: 'API',
 * //   status: 'unknown', latencyMs: null }], checkedAt } — 'unknown' until a check row exists.
 * ```
 *
 * @remarks
 * - **`requestHandlerMap` is not exported — call `createRequestHandlerMap(createRequestHandler)`.**
 *   Bond the DataStore (`setStore`) first; every handler reads/writes through it.
 * - **This package does NOT run health checks.** Nothing here writes `checks` or
 *   `uptimeWindows`; your own worker/cron must insert check rows (`serviceId`, `status`,
 *   `latencyMs`, `checkedAt`) — until then every service reports `'unknown'`.
 * - `createIncident` requires `serviceId` (a UUID), `title`, `severity`
 *   (`minor|major|critical`), `status` (`investigating|identified|monitoring|resolved`) and an
 *   ISO `startedAt`; timeouts/intervals are in MILLISECONDS (`timeoutMs`, `intervalMs`).
 * - Create/update responses wrap the row as `{ props }`; `requireAdmin` rejects via
 *   `next(message)`, so the status code comes from the app's error handler.
 *
