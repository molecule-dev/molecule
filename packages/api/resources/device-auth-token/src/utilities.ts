...
export const DEFAULT_PREFIX = 'dvt_'
 ...
 *
 * @param prefix - Optional token prefix (e.g. `'dvt_live_'`). Defaults to {@link DEFAULT_PREFIX}.
...
const inferPrefix = (plaintext: string): string => {
  // Recognize prefixes of the form `<lower>(_<lower>)*_` — e.g. `dvt_`,
  // `dvt_live_`, `iot_test_`. Avoids mis-inferring when the random tail
