/**
 * Browser guard — @molecule/api-resource-device-auth-token is SERVER-ONLY.
 *
...
if (g.window !== undefined && g.document !== undefined && !g.process?.versions?.node) {
  throw new Error(
    '@molecule/api-resource-device-auth-token is SERVER-ONLY: it was bundled into browser/client code. Import it only ' +
