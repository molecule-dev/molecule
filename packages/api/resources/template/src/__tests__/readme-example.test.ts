...
import { createTemplate, instantiateById } from '../index.js'

describe('README @example', () => {
  it('stores a template and instantiates it with defaults merged in', async () => {
    setStore(store)
    ...
    const template = await createTemplate({
      ...
      variables: [
        ...
        { name: 'greeting', defaultValue: 'Welcome' },
      ],
      createdBy: 'user-1',
    })
    expect(template).toMatchObject({ version: 1, isPublic: false, createdBy: 'user-1' })

    const result = await instantiateById(template.id, { name: 'Ada' })
    expect(result?.payload).toEqual({ title: 'Hello Ada', body: 'Welcome!' })
    expect(result?.missingVariables).toEqual([])

    const missing = await instantiateById(template.id)
    expect(missing?.missingVariables).toEqual(['name'])
    ...
    expect(tables.get('resource-templates')?.size).toBe(1)
  })
})
