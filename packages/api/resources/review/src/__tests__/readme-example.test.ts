...
import { createReview, getAverageRating, getReviewsByResource, markHelpful } from '../index.js'

describe('README @example', () => {
  it('creates reviews, votes helpful once and aggregates the rating', async () => {
    setStore(store)

    await createReview('product', 'prod-1', 'user-1', {
      rating: 5,
      title: 'Great',
      body: 'Love it',
    })
    ...
    await markHelpful(review.id, 'user-1')
    await markHelpful(review.id, 'user-1')
    ...
    expect(tables.get('reviews')?.get(review.id)?.helpful).toBe(1)
  })
})
