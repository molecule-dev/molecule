...
Generated: 2026-09-24T12:29:02.155Z
-->
...
import {
  ...
  useSafeArea,
  useTranslation,
} from '@molecule/app-react-native'
...
```typescript
function convergeHistory(prev: ChatMessage[], history: ChatMessage[]): ChatMessage[]
```

- `prev` — The store's current messages.
- `history` — The messages from the server's history endpoint.
...
#### `localOnlyMessages(prev, history)`
...
```typescript
function localOnlyMessages(prev: ChatMessage[], history: ChatMessage[]): ChatMessage[]
```

- `prev` — The store's current messages.
- `history` — The server's history.
