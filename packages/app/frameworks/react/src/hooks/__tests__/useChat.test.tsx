...
describe('useChat', () => {
  ...
  it('splits into a new assistant message at the plan→execute boundary', async () => {
    ...
    // Plan-phase output streams into the first assistant message.
    await act(async () => {
      emitText(0, 'Saving the plan…')
    })
    await waitFor(() => {
      const assistants = result.current.messages.filter((m) => m.role === 'assistant')
      expect(assistants[0]?.content).toBe('Saving the plan…')
    })

    // Mode flips to execute mid-stream → the message must split so phase-marker cards
    // can sort between the plan output and the execute output.
    await act(async () => {
      emit(0, { type: 'mode', mode: 'execute' })
    })

    // Execute-phase output streams into the NEW (second) message.
    await act(async () => {
      emitText(0, 'Let me start implementing.')
    })
    ...
    await waitFor(() => {
      ...
      expect(assistants[0].content).toBe('Saving the plan…')
      expect(assistants[1].content).toBe('Let me start implementing.')
    })

    // The plan message is finalized; only the execute message keeps streaming.
    const assistants = result.current.messages.filter((m) => m.role === 'assistant')
  ...
  it('does NOT split on a mode switch before any content (no empty leading message)', async () => {
    const { provider, emitText, emit, complete } = createMockProvider()
    const { result } = renderHook(
      () => useChat({ endpoint: ENDPOINT, projectId: PROJECT_ID, loadOnMount: false }),
      { wrapper: createWrapper(provider) },
    ...
    // Mode arrives with no prior content → must stay a single message.
    await act(async () => {
      emit(0, { type: 'mode', mode: 'execute' })
      emitText(0, 'Implementing.')
    })

    await waitFor(() => {
      const assistants = result.current.messages.filter((m) => m.role === 'assistant')
      expect(assistants).toHaveLength(1)
      expect(assistants[0].content).toBe('Implementing.')
    })
