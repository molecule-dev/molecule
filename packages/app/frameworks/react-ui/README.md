...
Generated: 2026-09-24T21:35:36.593Z
-->
...
```typescript
interface LoadErrorBannerProps extends HTMLElementProps {
...
```typescript
type ShellSidebarWidth = 'sm' | 'md' | 'lg' | number
```
...
- `props` — Component props (see {@link LoadErrorBannerProps}).
...
- `props` — Component props (see {@link PromptDialogProps}).
...
- `props` — The page content and class/id overrides.
...
- `props` — Nav children, header/nav-label/footer slots, close-button and class/id overrides.
...
- `props` — Nav children, nav label, footer slot, class/id overrides.
...
- `props` — Brand/actions slots, `mobileOnly`, and class/id overrides.
...
- `query` — Override the media query. Defaults to `(min-width: 768px)`.
...
```typescript
const ConfirmButton: React.ForwardRefExoticComponent<
...
```typescript
const ResponsiveAppShell: typeof ResponsiveAppShellBase & {
  TopBar: typeof ResponsiveAppShellTopBar
  Sidebar: typeof ResponsiveAppShellSidebar
  Drawer: typeof ResponsiveAppShellDrawer
  Content: typeof ResponsiveAppShellContent
}
```
