 ...
 * import { Alert, Button, Icon, Modal } from '@molecule/app-ui-react'
 * import { classMap } from '@molecule/app-ui-tailwind'
 *
 * // Once at startup (bonds/index.ts), before the first render — components throw without the ClassMap.
 ...
 *
 *   if (deleted) {
 *     return <Alert status="success">{t('common.completed', undefined, { defaultValue: 'Completed' })}</Alert>
 *   }
 *   return (
 *     <>
 *       <Button color="error" leftIcon={<Icon name="trash" size={16} />} onClick={() => setOpen(true)}>
 ...
 *   a runtime error, not a blank, and so is rendering any `Icon` before `setIconSet(iconSet)`
 *   (from `@molecule/app-icons-molecule`) has run. Extend via `CustomIconNames` augmentation, never raw SVG.
