 ...
 * }
 *
 * export function IdePage({ projectId }: { projectId: string }) {
 *   return (
 ...
 *
 * @remarks
 * - The panels are hook-driven and THROW without provider wiring. Pass ALL of
 *   `i18n`, `http`, `theme`, `chat`, `workspace`, `editor` and `preview` to
 *   `@molecule/app-react`'s `<MoleculeProvider>` (or nest the individual
 *   providers): `ChatPanel` and `EditorPanel` read `useThemeMode()` (no
 *   `theme` → throw), `ChatPanel` reads `useHttpClient()`. Typical bonds:
 *   `@molecule/app-ide-default` (workspace layout + persistence),
 *   `@molecule/app-code-editor-monaco`, `@molecule/app-ai-chat-http` and
 *   `@molecule/app-live-preview-iframe`. `setClassMap()` and `setIconSet()`
 *   must run before the first render.
 * - `ChatPanel` is a client of the molecule PLATFORM API, not a generic chat
 *   box: it loads and streams `/projects/:projectId/chat` through the chat
 *   provider (prefixed by its `baseUrl`) and calls `/projects/:projectId/...`
 *   (conversations, files, git status) and `/ai/models` through the HTTP client
 *   (prefixed by its `baseURL`). Point both at the same API root. For a plain
 *   chat widget use `@molecule/app-ai-chat` directly.
