...
export function PreviewPanel({ className }: PreviewPanelProps): JSX.Element {
  ...
  return (
    ...
    >
      ...
      >
        <span className={cm.cn(cm.fontWeight('medium'), cm.textSize('sm'))}>{t('ide.preview.title')}</span>
      ...
      {/* URL bar */}
      <div
        className={cm.cn(cm.sp('px', 3), cm.sp('py', 1), cm.shrink0, cm.borderB)}
      >
        <input
          type="text"
          value={state.url}
          onChange={(e) => setUrl(e.target.value)}
          className={cm.cn(cm.w('full'), cm.textSize('xs'), cm.sp('p', 1), cm.surfaceSecondary, cm.borderAll)}
          style={{
            borderRadius: '4px',
            color: 'inherit',
            outline: 'none',
      ...
      >
        {state.url ? (
          <iframe
            ...
            className={cm.cn(
              state.device !== 'none' && state.device !== 'desktop' && cm.borderAll,
            )}
            style={{
              width: iframeWidth,
              height: '100%',
