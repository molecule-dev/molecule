...
/**
 * Parses SVG path elements from an HTML string and extracts their attributes.
 * @param pathHtml - The HTML string containing SVG path elements.
 * @returns An array of parsed path objects with d, fillRule, and clipRule properties.
 */
// eslint-disable-next-line @typescript-eslint/explicit-function-return-type
 ...
 * @param moleculeName - The molecule icon name in kebab-case.
 * @param paths - The parsed SVG path data objects.
 * @param viewBox - The SVG viewBox attribute value.
...
/**
 * Generates a barrel (index) file that re-exports all icon modules.
 * @param allNames - The sorted array of all icon names to export.
 * @returns The generated barrel file content as a string.
 */
// eslint-disable-next-line @typescript-eslint/explicit-function-return-type
...
for (const [moleculeName, octiconName] of Object.entries(ICON_MAP)) {
    if (SKIP_ICONS.has(moleculeName))
        continue;
...
console.log(`Barrel file includes ${allBarrelNames.length} total icons (${SKIP_ICONS.size} hand-maintained)`);
