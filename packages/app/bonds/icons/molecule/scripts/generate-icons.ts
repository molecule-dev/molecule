...
const octicons = require('@primer/octicons') as Record<
  ...
  {
    symbol: string
    heights: Record<string, { path: string; width: number; options: { viewBox: string } }>
  }
>
 ...
 */
function parsePaths(pathHtml: string): ParsedPath[] {
  ...
  while ((match = pathRegex.exec(pathHtml)) !== null) {
    ...
    const parsed: ParsedPath = { d: dMatch[1]! }
 ...
 */
function generateIconFile(moleculeName: string, paths: ParsedPath[], viewBox: string): string {
 ...
 */
function generateBarrelFile(allNames: string[]): string {
