# @molecule/app-locales-exif-panel-react

Translations for @molecule/app-exif-panel-react in 79 languages

## Purpose

Provides translations for the `@molecule/app-exif-panel-react` package which has 17 translation keys.

## Languages

79 languages supported: af, am, ar, az, be, bg, bn, bs, ca, cs, cy, da, de, el, en, es, et, eu, fa, fi, fil, fr, ga, gl, gu, ha, he, hi, hr, hu, hy, id, ig, is, it, ja, ka, kk, km, kn, ko, ky, lo, lt, lv, mk, ml, mn, mr, ms, mt, my, nb, ne, nl, pa, pl, pt, ro, ru, si, sk, sl, sq, sr, sv, sw, ta, te, th, tr, uk, ur, uz, vi, yo, zh, zh-TW, zu.

## Quick Start

```typescript
import { af, am, ar } from '@molecule/app-locales-exif-panel-react'
import type { ExifPanelTranslationKey, ExifPanelTranslations } from '@molecule/app-locales-exif-panel-react'
```

## Translation Keys

| Key | English |
|-----|---------|
| `exifPanel.aria.region` | EXIF metadata |
| `exifPanel.eyebrow` | Frame metadata |
| `exifPanel.heading` | EXIF |
| `exifPanel.camera` | Camera |
| `exifPanel.lens` | Lens |
| `exifPanel.exposure` | Exposure |
| `exifPanel.aperture` | Aperture |
| `exifPanel.shutter` | Shutter speed |
| `exifPanel.iso` | ISO |
| `exifPanel.focalLength` | Focal length |
| `exifPanel.focalLength35mm` | 35 mm equivalent |
| `exifPanel.gps` | GPS |
| `exifPanel.timestamp` | Captured |
| `exifPanel.software` | Software |
| `exifPanel.copyright` | Copyright |
| `exifPanel.orientation` | Orientation |
| `exifPanel.mapLink` | View on map |

## Metadata

- **Type:** locales
- **Category:** i18n
- **Stack:** app
- **Translates:** `@molecule/app-exif-panel-react`
