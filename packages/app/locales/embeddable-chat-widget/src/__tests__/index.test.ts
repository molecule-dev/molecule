...
const reference = locales.en as EmbeddableChatWidgetTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof EmbeddableChatWidgetTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, EmbeddableChatWidgetTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('embeddable-chat-widget locale bond', () => {
