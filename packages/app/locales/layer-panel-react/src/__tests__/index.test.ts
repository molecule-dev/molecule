...
const reference = locales.en as LayerPanelTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof LayerPanelTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, LayerPanelTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('layer-panel-react locale bond', () => {
