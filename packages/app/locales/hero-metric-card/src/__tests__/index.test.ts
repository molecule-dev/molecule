...
import type { HeroMetricCardTranslations } from '../types.js'

const reference = locales.en as HeroMetricCardTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof HeroMetricCardTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, HeroMetricCardTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('hero-metric-card locale bond', () => {
