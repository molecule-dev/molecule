...
const reference = locales.en as PushTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof PushTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, PushTranslations] => entry[1] !== null && typeof entry[1] === 'object',
)

describe('push locale bond', () => {
