...
const reference = locales.en as StatusDashboardTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof StatusDashboardTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, StatusDashboardTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('status-dashboard locale bond', () => {
