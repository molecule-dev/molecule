...
const reference = locales.en as FilesystemTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof FilesystemTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, FilesystemTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('filesystem locale bond', () => {
