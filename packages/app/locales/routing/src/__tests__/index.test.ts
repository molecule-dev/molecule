...
const reference = locales.en as RoutingTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof RoutingTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, RoutingTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('routing locale bond', () => {
