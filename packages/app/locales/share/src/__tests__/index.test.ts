...
const reference = locales.en as ShareTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof ShareTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, ShareTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('share locale bond', () => {
