...
const reference = locales.en as StatusBarTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof StatusBarTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, StatusBarTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('status-bar locale bond', () => {
