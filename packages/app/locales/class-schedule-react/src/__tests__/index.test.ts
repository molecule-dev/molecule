...
const reference = locales.en as ClassScheduleTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof ClassScheduleTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, ClassScheduleTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('class-schedule-react locale bond', () => {
