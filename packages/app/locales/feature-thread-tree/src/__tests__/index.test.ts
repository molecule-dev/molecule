...
const reference = locales.en as ThreadTreeTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof ThreadTreeTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, ThreadTreeTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('feature-thread-tree-react locale bond', () => {
