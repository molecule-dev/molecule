...
const reference = locales.en as LivePreviewTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof LivePreviewTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, LivePreviewTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('live-preview locale bond', () => {
