import type { FeatureVideoTimelineTranslations } from './types.js'

/** FeatureVideoTimeline translations for si. */
export const si: Partial<FeatureVideoTimelineTranslations> = {
  'videoTimeline.aria.root': 'වීඩියෝ කාලරේඛාව',
  'videoTimeline.aria.ruler': 'කාල පාලකයා',
  'videoTimeline.aria.playhead': 'ප්ලේහෙඩ්<x> {{වේලාව}}</x> එස්',
  'videoTimeline.aria.zoom': 'කාලරේඛා විශාලනය',
  'videoTimeline.aria.mode': 'සංස්කරණ ප්‍රකාරය',
  'videoTimeline.zoom.in': 'විශාලනය කරන්න',
  'videoTimeline.zoom.out': 'විශාලනය අඩු කරන්න',
  'videoTimeline.zoom.in.icon': '+',
  'videoTimeline.zoom.out.icon': '−',
  'videoTimeline.mode.ripple': 'රැළිය',
  'videoTimeline.mode.insert': 'ඇතුළු කරන්න',
  'videoTimeline.trackKind.video': 'වීඩියෝ',
  'videoTimeline.trackKind.audio': 'ශ්‍රව්‍ය',
  'videoTimeline.trackKind.subtitle': 'උපසිරැසි',
}
