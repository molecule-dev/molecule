import type { FeatureVideoTimelineTranslations } from './types.js'

/** FeatureVideoTimeline translations for he. */
export const he: Partial<FeatureVideoTimelineTranslations> = {
  'videoTimeline.mode.insert': 'לְהַכנִיס',
  'videoTimeline.aria.root': 'ציר זמן של וידאו',
  'videoTimeline.aria.ruler': 'שליט הזמן',
  'videoTimeline.aria.playhead': 'ראש השמעה ב<x> {{זְמַן}}</x> ש',
  'videoTimeline.aria.zoom': 'זום של ציר הזמן',
  'videoTimeline.aria.mode': 'מצב עריכה',
  'videoTimeline.zoom.in': 'לְהִתְמַקֵד',
  'videoTimeline.zoom.out': 'התרחקות',
  'videoTimeline.zoom.in.icon': '+',
  'videoTimeline.zoom.out.icon': '−',
  'videoTimeline.mode.ripple': 'אַדְוָה',
  'videoTimeline.trackKind.video': 'וִידֵאוֹ',
  'videoTimeline.trackKind.audio': 'שֶׁמַע',
  'videoTimeline.trackKind.subtitle': 'כְּתוּבִית',
}
