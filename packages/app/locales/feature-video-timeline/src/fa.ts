import type { FeatureVideoTimelineTranslations } from './types.js'

/** FeatureVideoTimeline translations for fa. */
export const fa: Partial<FeatureVideoTimelineTranslations> = {
  'videoTimeline.aria.root': 'جدول زمانی ویدیو',
  'videoTimeline.aria.ruler': 'خط کش زمان',
  'videoTimeline.aria.playhead': 'پلی‌هد در<x> {{زمان}}</x> ها',
  'videoTimeline.aria.zoom': 'بزرگنمایی تایم‌لاین',
  'videoTimeline.aria.mode': 'حالت ویرایش',
  'videoTimeline.zoom.in': 'بزرگنمایی',
  'videoTimeline.zoom.out': 'کوچک‌نمایی',
  'videoTimeline.zoom.in.icon': '+',
  'videoTimeline.zoom.out.icon': '−',
  'videoTimeline.mode.ripple': 'موج دار شدن',
  'videoTimeline.mode.insert': 'درج',
  'videoTimeline.trackKind.video': 'ویدئو',
  'videoTimeline.trackKind.audio': 'صوتی',
  'videoTimeline.trackKind.subtitle': 'زیرنویس',
}
