import type { FeatureVideoTimelineTranslations } from './types.js'

/** FeatureVideoTimeline translations for ta. */
export const ta: Partial<FeatureVideoTimelineTranslations> = {
  'videoTimeline.aria.root': 'வீடியோ காலவரிசை',
  'videoTimeline.aria.ruler': 'நேர ஆட்சியாளர்',
  'videoTimeline.aria.playhead': 'பிளேஹெட் அட்<x> {{நேரம்}}</x> s',
  'videoTimeline.aria.zoom': 'காலவரிசை பெரிதாக்கம்',
  'videoTimeline.aria.mode': 'திருத்து முறை',
  'videoTimeline.zoom.in': 'பெரிதாக்கு',
  'videoTimeline.zoom.out': 'பெரிதாக்கிக் காட்டு',
  'videoTimeline.zoom.in.icon': '+',
  'videoTimeline.zoom.out.icon': '−',
  'videoTimeline.mode.ripple': 'சிற்றலை',
  'videoTimeline.mode.insert': 'சேர்க்கவும்',
  'videoTimeline.trackKind.video': 'வீடியோ',
  'videoTimeline.trackKind.audio': 'ஆடியோ',
  'videoTimeline.trackKind.subtitle': 'துணைத் தலைப்பு',
}
