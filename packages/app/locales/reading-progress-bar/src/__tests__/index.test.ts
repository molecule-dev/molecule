...
import type { ReadingProgressBarTranslations } from '../types.js'

const reference = locales.en as ReadingProgressBarTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof ReadingProgressBarTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, ReadingProgressBarTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('reading-progress-bar-react locale bond', () => {
