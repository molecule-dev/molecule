...
import type { SurveyQuestionTranslations } from '../types.js'

const reference = locales.en as SurveyQuestionTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof SurveyQuestionTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, SurveyQuestionTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('survey-question-react locale bond', () => {
