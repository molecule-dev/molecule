# @molecule/app-locales-survey-question-react

Translations for @molecule/app-survey-question-react in 79 languages

## Purpose

Provides translations for the `@molecule/app-survey-question-react` package which has 8 translation keys.

## Languages

79 languages supported: af, am, ar, az, be, bg, bn, bs, ca, cs, cy, da, de, el, en, es, et, eu, fa, fi, fil, fr, ga, gl, gu, ha, he, hi, hr, hu, hy, id, ig, is, it, ja, ka, kk, km, kn, ko, ky, lo, lt, lv, mk, ml, mn, mr, ms, mt, my, nb, ne, nl, pa, pl, pt, ro, ru, si, sk, sl, sq, sr, sv, sw, ta, te, th, tr, uk, ur, uz, vi, yo, zh, zh-TW, zu.

## Quick Start

```typescript
import { af, am, ar } from '@molecule/app-locales-survey-question-react'
import type { SurveyQuestionTranslationKey, SurveyQuestionTranslations } from '@molecule/app-locales-survey-question-react'
```

## Translation Keys

| Key | English |
|-----|---------|
| `surveyQuestion.required` | This question requires an answer. |
| `surveyQuestion.requiredIndicator` | required |
| `surveyQuestion.submit` | Submit |
| `surveyQuestion.trueFalse.true` | True |
| `surveyQuestion.trueFalse.false` | False |
| `surveyQuestion.nps.low` | Not at all likely |
| `surveyQuestion.nps.high` | Extremely likely |
| `surveyQuestion.file.noFiles` | No files selected. |

## Metadata

- **Type:** locales
- **Category:** i18n
- **Stack:** app
- **Translates:** `@molecule/app-survey-question-react`
