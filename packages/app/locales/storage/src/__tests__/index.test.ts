...
const reference = locales.en as StorageTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof StorageTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, StorageTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('storage locale bond', () => {
