...
const reference = locales.en as VueTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof VueTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, VueTranslations] => entry[1] !== null && typeof entry[1] === 'object',
)

describe('vue locale bond', () => {
