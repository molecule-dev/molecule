...
const reference = locales.en as FileCardTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof FileCardTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, FileCardTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('file-card locale bond', () => {
