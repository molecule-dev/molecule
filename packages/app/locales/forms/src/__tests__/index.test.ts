...
const reference = locales.en as FormsTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof FormsTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, FormsTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('forms locale bond', () => {
