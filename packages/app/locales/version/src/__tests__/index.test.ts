...
const reference = locales.en as VersionTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof VersionTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, VersionTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('version locale bond', () => {
