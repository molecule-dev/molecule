...
import type { FlowCanvasTranslations } from '../types.js'

const reference = locales.en as FlowCanvasTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof FlowCanvasTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, FlowCanvasTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('flow-canvas-react locale bond', () => {
