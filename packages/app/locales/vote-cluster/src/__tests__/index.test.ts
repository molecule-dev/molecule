...
const reference = locales.en as VoteClusterTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof VoteClusterTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, VoteClusterTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('vote-cluster locale bond', () => {
