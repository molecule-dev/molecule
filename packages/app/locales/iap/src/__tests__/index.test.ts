...
const reference = locales.en as IapTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof IapTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, IapTranslations] => entry[1] !== null && typeof entry[1] === 'object',
)

describe('iap locale bond', () => {
