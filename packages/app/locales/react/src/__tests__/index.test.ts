...
const reference = locales.en as ReactTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof ReactTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, ReactTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('react locale bond', () => {
