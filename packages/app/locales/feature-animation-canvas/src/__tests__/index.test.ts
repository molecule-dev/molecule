...
const reference = locales.en as AnimationCanvasTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof AnimationCanvasTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, AnimationCanvasTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('feature-animation-canvas-react locale bond', () => {
