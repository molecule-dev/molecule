...
const reference = locales.en as MotionTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof MotionTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, MotionTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('motion locale bond', () => {
