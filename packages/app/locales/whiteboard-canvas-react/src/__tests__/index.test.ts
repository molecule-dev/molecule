...
import type { WhiteboardCanvasTranslations } from '../types.js'

const reference = locales.en as WhiteboardCanvasTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof WhiteboardCanvasTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, WhiteboardCanvasTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('whiteboard-canvas-react locale bond', () => {
