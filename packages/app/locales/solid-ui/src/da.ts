import type { SolidUiTranslations } from './types.js'

/** SolidUi translations for da. */
export const da: Partial<SolidUiTranslations> = {
  'ui.alert.dismiss': 'Afvis',
  'ui.input.clear': 'Ryd',
  'ui.modal.close': 'Luk',
  'ui.progress.label': 'Fremskridt',
  'ui.toast.close': 'Luk',
  'solid.error.useAccordionOutsideProvider': 'Harmonikakomponenter skal bruges i et harmonika',
  'ui.avatar.alt': 'Avatar',
  'ui.pagination.nav': 'Paginering',
  'ui.pagination.first': 'Gå til første side',
  'ui.pagination.previous': 'Gå til forrige side',
  'ui.pagination.goToPage': 'Gå til side {{page}}',
  'ui.pagination.last': 'Gå til sidste side',
  'ui.radioGroup.label': 'Radiogruppe',
  'ui.spinner.loading': 'Indlæser',
  'ui.table.empty': 'Ingen data tilgængelige',
  'solid.error.useToastOutsideProvider': 'useToast skal bruges i en ToastProvider',
}
