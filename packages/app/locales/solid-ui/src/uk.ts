import type { SolidUiTranslations } from './types.js'

/** SolidUi translations for uk. */
export const uk: Partial<SolidUiTranslations> = {
  'ui.alert.dismiss': 'Закрити',
  'ui.avatar.alt': 'Аватар',
  'ui.input.clear': 'Очистити',
  'ui.modal.close': 'Закрити',
  'ui.progress.label': 'Прогрес',
  'ui.toast.close': 'Закрити',
  'solid.error.useAccordionOutsideProvider':
    'Компоненти акордеона повинні використовуватися всередині акордеона',
  'ui.pagination.nav': 'Пагінація',
  'ui.pagination.first': 'Перейти на першу сторінку',
  'ui.pagination.previous': 'Перейти на попередню сторінку',
  'ui.pagination.goToPage': 'Перейти на сторінку {{page}}',
  'ui.pagination.last': 'Перейти на останню сторінку',
  'ui.radioGroup.label': 'Радіогрупа',
  'ui.spinner.loading': 'Завантаження',
  'ui.table.empty': 'Дані недоступні',
  'solid.error.useToastOutsideProvider': 'useToast має використовуватися всередині ToastProvider',
}
