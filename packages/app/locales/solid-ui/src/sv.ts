import type { SolidUiTranslations } from './types.js'

/** SolidUi translations for sv. */
export const sv: Partial<SolidUiTranslations> = {
  'ui.alert.dismiss': 'Avfärda',
  'ui.input.clear': 'Rensa',
  'ui.modal.close': 'Stäng',
  'ui.progress.label': 'Framsteg',
  'ui.spinner.loading': 'Läser in',
  'ui.toast.close': 'Stäng',
  'solid.error.useAccordionOutsideProvider': 'Dragspelskomponenter måste användas i ett dragspel',
  'ui.avatar.alt': 'Avatar',
  'ui.pagination.nav': 'Paginering',
  'ui.pagination.first': 'Gå till första sidan',
  'ui.pagination.previous': 'Gå till föregående sida',
  'ui.pagination.goToPage': 'Gå till sidan {{page}}',
  'ui.pagination.last': 'Gå till sista sidan',
  'ui.radioGroup.label': 'Radiogrupp',
  'ui.table.empty': 'Inga data tillgängliga',
  'solid.error.useToastOutsideProvider': 'useToast måste användas inom en ToastProvider',
}
