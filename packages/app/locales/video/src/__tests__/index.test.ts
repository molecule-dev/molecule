...
const reference = locales.en as VideoTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof VideoTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, VideoTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('video locale bond', () => {
