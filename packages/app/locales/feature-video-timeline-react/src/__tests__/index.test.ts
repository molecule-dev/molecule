...
const reference = locales.en as VideoTimelineTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof VideoTimelineTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, VideoTimelineTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('feature-video-timeline-react locale bond', () => {
