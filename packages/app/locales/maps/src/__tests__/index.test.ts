...
const reference = locales.en as MapsTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof MapsTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, MapsTranslations] => entry[1] !== null && typeof entry[1] === 'object',
)

describe('maps locale bond', () => {
