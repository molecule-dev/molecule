...
const reference = locales.en as BarcodeScannerTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof BarcodeScannerTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, BarcodeScannerTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('feature-barcode-scanner-react locale bond', () => {
