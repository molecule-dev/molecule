...
const reference = locales.en as ImageCanvasTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof ImageCanvasTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, ImageCanvasTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('feature-image-canvas-react locale bond', () => {
