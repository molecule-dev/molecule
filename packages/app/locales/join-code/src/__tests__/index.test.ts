...
const reference = locales.en as JoinCodeTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof JoinCodeTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, JoinCodeTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('join-code locale bond', () => {
