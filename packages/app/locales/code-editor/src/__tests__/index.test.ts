...
const reference = locales.en as CodeEditorTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof CodeEditorTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, CodeEditorTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('code-editor locale bond', () => {
