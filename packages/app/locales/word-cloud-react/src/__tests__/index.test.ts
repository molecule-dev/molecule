...
import type { WordCloudTranslations } from '../types.js'

const reference = locales.en as WordCloudTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof WordCloudTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, WordCloudTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('word-cloud-react locale bond', () => {
