...
import type { ChartsTranslations } from '../types.js'

const reference = locales.en as ChartsTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof ChartsTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, ChartsTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('charts locale bond', () => {
