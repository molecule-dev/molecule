# @molecule/app-locales-author-bio-card-react

Companion locale bond for `@molecule/app-author-bio-card-react` —
provides translation strings for the `<AuthorBioCard>` component in
79 languages.

## Purpose

Provides translations for the `@molecule/app-author-bio-card-react` package which has 12 translation keys.

## Languages

79 languages supported: af, am, ar, az, be, bg, bn, bs, ca, cs, cy, da, de, el, en, es, et, eu, fa, fi, fil, fr, ga, gl, gu, ha, he, hi, hr, hu, hy, id, ig, is, it, ja, ka, kk, km, kn, ko, ky, lo, lt, lv, mk, ml, mn, mr, ms, mt, my, nb, ne, nl, pa, pl, pt, ro, ru, si, sk, sl, sq, sr, sv, sw, ta, te, th, tr, uk, ur, uz, vi, yo, zh, zh-TW, zu.

## Quick Start

```typescript
import { af, am, ar } from '@molecule/app-locales-author-bio-card-react'
import type { AuthorBioCardTranslationKey, AuthorBioCardTranslations } from '@molecule/app-locales-author-bio-card-react'
```

## Translation Keys

| Key | English |
|-----|---------|
| `authorBioCard.follow` | Follow |
| `authorBioCard.following` | Following |
| `authorBioCard.social.twitter.label` | Twitter |
| `authorBioCard.social.github.label` | GitHub |
| `authorBioCard.social.linkedin.label` | LinkedIn |
| `authorBioCard.social.mastodon.label` | Mastodon |
| `authorBioCard.social.website.label` | Website |
| `authorBioCard.social.twitter` | {{name}} on Twitter |
| `authorBioCard.social.github` | {{name}} on GitHub |
| `authorBioCard.social.linkedin` | {{name}} on LinkedIn |
| `authorBioCard.social.mastodon` | {{name}} on Mastodon |
| `authorBioCard.social.website` | {{name}} |

## Metadata

- **Type:** locales
- **Category:** i18n
- **Stack:** app
- **Translates:** `@molecule/app-author-bio-card-react`
