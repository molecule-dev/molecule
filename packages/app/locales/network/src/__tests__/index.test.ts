...
const reference = locales.en as NetworkTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof NetworkTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, NetworkTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('network locale bond', () => {
