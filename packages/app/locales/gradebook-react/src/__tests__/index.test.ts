...
const reference = locales.en as GradebookTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof GradebookTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, GradebookTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('gradebook-react locale bond', () => {
