...
const reference = locales.en as MapDrawingTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof MapDrawingTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, MapDrawingTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('feature-map-drawing-react locale bond', () => {
