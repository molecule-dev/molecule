...
const reference = locales.en as MindMapCanvasTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof MindMapCanvasTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, MindMapCanvasTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('mind-map-canvas-react locale bond', () => {
