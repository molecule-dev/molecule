...
import type { NpsDistributionTranslations } from '../types.js'

const reference = locales.en as NpsDistributionTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof NpsDistributionTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, NpsDistributionTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('nps-distribution locale bond', () => {
