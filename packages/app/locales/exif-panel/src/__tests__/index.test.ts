...
const reference = locales.en as ExifPanelTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof ExifPanelTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, ExifPanelTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('exif-panel-react locale bond', () => {
