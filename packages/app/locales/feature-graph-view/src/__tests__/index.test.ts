...
const reference = locales.en as GraphViewTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof GraphViewTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, GraphViewTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('feature-graph-view-react locale bond', () => {
