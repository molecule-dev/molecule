...
const reference = locales.en as ContactsTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof ContactsTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, ContactsTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('contacts locale bond', () => {
