...
const reference = locales.en as TransportBarTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof TransportBarTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, TransportBarTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('feature-transport-bar-react locale bond', () => {
