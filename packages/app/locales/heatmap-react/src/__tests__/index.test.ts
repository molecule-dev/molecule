...
const reference = locales.en as HeatmapTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof HeatmapTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, HeatmapTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('heatmap-react locale bond', () => {
