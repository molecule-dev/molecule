...
const reference = locales.en as ScreenOrientationTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof ScreenOrientationTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, ScreenOrientationTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('screen-orientation locale bond', () => {
