...
const reference = locales.en as NfcTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof NfcTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, NfcTranslations] => entry[1] !== null && typeof entry[1] === 'object',
)

describe('nfc locale bond', () => {
