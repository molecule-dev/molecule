...
import type { SpreadsheetGridTranslations } from '../types.js'

const reference = locales.en as SpreadsheetGridTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof SpreadsheetGridTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, SpreadsheetGridTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('spreadsheet-grid-react locale bond', () => {
