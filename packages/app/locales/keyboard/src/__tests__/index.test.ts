...
const reference = locales.en as KeyboardTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof KeyboardTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, KeyboardTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('keyboard locale bond', () => {
