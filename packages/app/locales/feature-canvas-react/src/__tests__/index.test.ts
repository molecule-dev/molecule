...
const reference = locales.en as CanvasTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof CanvasTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, CanvasTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('feature-canvas-react locale bond', () => {
