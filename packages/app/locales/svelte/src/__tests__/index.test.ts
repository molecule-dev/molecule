...
const reference = locales.en as SvelteTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof SvelteTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, SvelteTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('svelte locale bond', () => {
