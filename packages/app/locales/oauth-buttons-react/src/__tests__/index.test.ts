...
import type { OAuthButtonsTranslations } from '../types.js'

const reference = locales.en as OAuthButtonsTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof OAuthButtonsTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, OAuthButtonsTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('oauth-buttons-react locale bond', () => {
