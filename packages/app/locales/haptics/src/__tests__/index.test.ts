...
const reference = locales.en as HapticsTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof HapticsTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, HapticsTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('haptics locale bond', () => {
