import type { ReactUiTranslations } from './types.js'

/** ReactUi translations for ml. */
export const ml: Partial<ReactUiTranslations> = {
  'userMenu.navLabel': 'അക്കൗണ്ട് മെനു',
  'react.error.useAccordionOutsideProvider': 'ഒരു അക്കോഡിയനുള്ളിൽ അക്കോഡിയൻ ഘടകങ്ങൾ ഉപയോഗിക്കണം.',
  'ui.alert.dismiss': 'നിരസിക്കുക',
  'ui.avatar.alt': 'അവതാർ',
  'ui.input.clear': 'വ്യക്തം',
  'ui.modal.close': 'അടയ്ക്കുക',
  'ui.pagination.nav': 'പേജിനേഷൻ',
  'ui.pagination.first': 'ആദ്യ പേജിലേക്ക് പോകുക',
  'ui.pagination.previous': 'മുമ്പത്തെ പേജിലേക്ക് പോകുക',
  'ui.pagination.goToPage': 'പേജിലേക്ക് പോകുക {{page}}',
  'ui.pagination.last': 'അവസാന പേജിലേക്ക് പോകുക',
  'ui.progress.label': 'പുരോഗതി',
  'ui.radioGroup.label': 'റേഡിയോ ഗ്രൂപ്പ്',
  'sidebar.memberStatus': 'പ്രീമിയം അംഗം',
  'sidebarUserCard.guest': 'അതിഥി',
  'ui.spinner.loading': 'ലോഡ് ചെയ്യുന്നു',
  'ui.table.empty': 'ഡാറ്റ ലഭ്യമല്ല.',
  'ui.toast.close': 'അടയ്ക്കുക',
  'react.error.useToastOutsideProvider': 'useToast ഒരു ToastProvider-ൽ തന്നെ ഉപയോഗിക്കണം.',
  'userMenu.guestName': 'അനലിസ്റ്റ്',
  'userMenuPopover.guest': 'അക്കൗണ്ട്',
}
