import type { ReactUiTranslations } from './types.js'

/** ReactUi translations for hi. */
export const hi: Partial<ReactUiTranslations> = {
  'ui.alert.dismiss': 'खारिज करें',
  'ui.avatar.alt': 'अवतार',
  'ui.input.clear': 'साफ़ करें',
  'ui.modal.close': 'बंद करें',
  'ui.progress.label': 'प्रगति',
  'sidebarUserCard.guest': 'अतिथि',
  'ui.spinner.loading': 'लोड हो रहा है',
  'ui.toast.close': 'बंद करें',
  'userMenu.guestName': 'विश्लेषक',
  'userMenuPopover.guest': 'खाता',
  'userMenu.navLabel': 'खाता मेनू',
  'react.error.useAccordionOutsideProvider':
    'अकॉर्डियन के घटकों का उपयोग अकॉर्डियन के भीतर ही किया जाना चाहिए।',
  'ui.pagination.nav': 'पृष्ठ पर अंक लगाना',
  'ui.pagination.first': 'पहले पृष्ठ पर जाएं',
  'ui.pagination.previous': 'पिछले पृष्ठ पर जाएं',
  'ui.pagination.goToPage': 'पृष्ठ पर जाओ {{page}}',
  'ui.pagination.last': 'अंतिम पृष्ठ पर जाएं',
  'ui.radioGroup.label': 'रेडियो समूह',
  'sidebar.memberStatus': 'प्रीमियम सदस्य',
  'ui.table.empty': 'कोई डेटा मौजूद नहीं',
  'react.error.useToastOutsideProvider':
    'useToast का उपयोग ToastProvider के भीतर ही किया जाना चाहिए।',
}
