import type { ReactUiTranslations } from './types.js'

/** ReactUi translations for es. */
export const es: Partial<ReactUiTranslations> = {
  'ui.alert.dismiss': 'Descartar',
  'ui.input.clear': 'Borrar',
  'ui.modal.close': 'Cerrar',
  'ui.progress.label': 'Progreso',
  'sidebarUserCard.guest': 'Invitado',
  'ui.spinner.loading': 'Cargando',
  'ui.toast.close': 'Cerrar',
  'userMenu.guestName': 'Analista',
  'userMenuPopover.guest': 'Cuenta',
  'userMenu.navLabel': 'Menú de cuenta',
  'react.error.useAccordionOutsideProvider':
    'Los componentes del acordeón deben usarse dentro de un acordeón.',
  'ui.avatar.alt': 'Avatar',
  'ui.pagination.nav': 'Paginación',
  'ui.pagination.first': 'Ir a la primera página',
  'ui.pagination.previous': 'Ir a la página anterior',
  'ui.pagination.goToPage': 'Ir a la página {{page}}',
  'ui.pagination.last': 'Ir a la última página',
  'ui.radioGroup.label': 'Grupo de radio',
  'sidebar.memberStatus': 'Miembro Premium',
  'ui.table.empty': 'No hay datos disponibles',
  'react.error.useToastOutsideProvider': 'useToast debe usarse dentro de un ToastProvider',
}
