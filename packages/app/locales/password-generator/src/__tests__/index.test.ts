...
const reference = locales.en as PasswordGeneratorTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof PasswordGeneratorTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, PasswordGeneratorTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('password-generator locale bond', () => {
