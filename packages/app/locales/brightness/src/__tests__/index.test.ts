...
import type { BrightnessTranslationKey, BrightnessTranslations } from '../types.js'

const REQUIRED_KEYS: BrightnessTranslationKey[] = ['brightness.error.noProvider']

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, BrightnessTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('brightness locale bond', () => {
