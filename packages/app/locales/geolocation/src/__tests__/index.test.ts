...
const reference = locales.en as GeolocationTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof GeolocationTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, GeolocationTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('geolocation locale bond', () => {
