...
const reference = locales.en as EmbedSnippetTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof EmbedSnippetTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, EmbedSnippetTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('embed-snippet-react locale bond', () => {
