...
const reference = locales.en as HealthTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof HealthTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, HealthTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('health locale bond', () => {
