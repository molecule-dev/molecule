...
const reference = locales.en as AudioMixerTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof AudioMixerTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, AudioMixerTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('feature-audio-mixer-react locale bond', () => {
