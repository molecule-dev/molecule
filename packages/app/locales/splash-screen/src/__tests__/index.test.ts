...
const reference = locales.en as SplashScreenTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof SplashScreenTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, SplashScreenTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('splash-screen locale bond', () => {
