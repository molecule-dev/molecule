...
import type { HttpTranslations } from '../types.js'

const reference = locales.en as HttpTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof HttpTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, HttpTranslations] => entry[1] !== null && typeof entry[1] === 'object',
)

describe('http locale bond', () => {
