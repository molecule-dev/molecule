...
const reference = locales.en as ProgressRingsTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof ProgressRingsTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, ProgressRingsTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('progress-rings-react locale bond', () => {
