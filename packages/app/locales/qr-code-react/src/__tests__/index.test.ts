...
const reference = locales.en as QrCodeTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof QrCodeTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, QrCodeTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('qr-code-react locale bond', () => {
