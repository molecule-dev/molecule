...
const reference = locales.en as SolidTranslations
const REQUIRED_KEYS = Object.keys(reference) as (keyof SolidTranslations)[]

const localeTables = Object.entries(locales).filter(
  (entry): entry is [string, SolidTranslations] =>
    entry[1] !== null && typeof entry[1] === 'object',
)

describe('solid locale bond', () => {
