...
describe('@molecule/app-code-editor provider', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
  ...
  it('should set provider via bond', () => {
    ...
    setProvider(mock)
    expect(mockBond).toHaveBeenCalledWith('code-editor', mock)
  })
  ...
  it('should throw when requiring absent provider', () => {
    mockBondGet.mockReturnValue(undefined)
    expect(() => requireProvider()).toThrow('Code editor provider not configured')
  })
})
