...
describe('@molecule/app-live-preview provider', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
  ...
  it('should set provider via bond', () => {
    ...
    const mock = { name: 'test' } as any
    setProvider(mock)
    expect(mockBond).toHaveBeenCalledWith('live-preview', mock)
  })
  ...
  it('should throw when requiring absent provider', () => {
    mockBondGet.mockReturnValue(undefined)
    expect(() => requireProvider()).toThrow('Live preview provider not configured')
  })
})
